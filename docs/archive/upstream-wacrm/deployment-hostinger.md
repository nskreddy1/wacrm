# Deploy on Hostinger (upstream snapshot)

> Source: https://wacrm.tech/docs/deployment-hostinger
...
1. Sign up at https://www.hostinger.com/web-apps-hosting and choose a Node.js plan that gives you enough memory for `npm run build` (2 GB+ recommended).
2. During the onboarding wizard, pick the Node.js stack and the region closest to your users.
...
1. Create repository → paste your fork's HTTPS URL (e.g., `https://github.com/<you>/wacrm.git`).
...
2. Click Run NPM Build (or execute `npm run build` from the app terminal).
...
- Outside — any uptime monitor (UptimeRobot, Better Stack, GitHub Actions) can hit the URL once a minute.
