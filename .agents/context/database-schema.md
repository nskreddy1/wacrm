...
`pg_policies`). **91 tables** in schema `public`,
built by the **136 migrations** in `supabase/migrations/`.
...
Tables outside `public` are not expanded here: `auth` (23), `cron` (2), `net` (2), `realtime` (10), `storage` (8), `vault` (1), `wacrm_internal` (1). Inspect
...
Indexes:
- `idx_webhook_events_processed_at`: CREATE INDEX idx_webhook_events_processed_at ON public.webhook_events USING btree (processed_at)
- `webhook_events_pkey`: CREATE UNIQUE INDEX webhook_events_pkey ON public.webhook_events USING btree (event_id)
