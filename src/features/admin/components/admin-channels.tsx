...
const CHANNEL_PROVIDERS: Record<
  ChannelKind,
  { value: ChannelProvider; label: string }[]
> = {
  whatsapp: [
...
export function AdminChannels() {
  ...
  return (
    <section
      ...
      aria-label="Client channel configuration"
    >
...
function ChannelTabs({
  ...
  const byChannel = useMemo(() => {
    const map: Record<ChannelKind, Connection[]> = {
      whatsapp: [],
      sms: [],
      email: [],
    };
    for (const connection of connections)
      map[connection.channel]?.push(connection);
    return map;
  }, [connections]);
        ...
        {CHANNELS.map((channel) => (
          <TabsTrigger key={channel} value={channel}>
            ...
            {byChannel[channel].length > 0 ? (
              <span className="bg-muted text-muted-foreground ml-1.5 rounded-full px-1.5 text-xs">
...
function ConnectionRow({
  ...
  async function patch(payload: Record<string, unknown>, key: string) {
    ...
    try {
      ...
      const body = await res.json().catch(() => null);
      if (!res.ok)
        throw new Error(body?.error ?? body?.health?.error ?? 'Request failed');
      onChanged();
      return true;
    } catch (err) {
  ...
  return (
    <article className="border-border bg-card flex flex-wrap items-center gap-3 rounded-lg border px-4 py-3">
      <div className="bg-primary-soft flex size-9 shrink-0 items-center justify-center rounded-md">
      ...
      <div className="flex items-center gap-2">
        ...
        <label className="text-muted-foreground flex items-center gap-2 text-xs">
          ...
          <Switch
            ...
            onCheckedChange={(checked) =>
              void patch({ isEnabled: checked }, 'toggle')
            }
          />
        </label>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger
            className="hover:bg-accent hover:text-accent-foreground flex size-8 items-center justify-center rounded-md"
...
function ProvisionSheet({
  ...
  if (open !== lastOpen) {
    ...
    if (open) {
      setProvider(
        (connection?.provider as ChannelProvider) ?? providerOptions[0].value
      );
      setForm({
        ...FORM_DEFAULTS,
    ...
    try {
      ...
      const res = await fetch('/api/admin/channels', {
        ...
        body: JSON.stringify({
          ...
          ...(editing && !isPlatformManaged && takeover
            ? { takeover: true }
            : {}),
        }),
      });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader>
          <SheetTitle>
            {editing
              ? `Edit ${CHANNEL_LABEL[targetChannel]} connection`
              : `Provision ${CHANNEL_LABEL[targetChannel]} connection`}
          </SheetTitle>
          <SheetDescription>
            {editing
        ...
        <div className="flex flex-1 flex-col gap-3 px-4 pb-4">
            ...
            <>
              <div className="border-border bg-muted/40 flex items-center gap-4 rounded-md border px-4 py-3">
                ...
                <Input
                  ...
                  placeholder={
                    editing ? 'Unchanged' : 'Permanent token (EAAG…)'
                  }
                  autoComplete="new-password"
                  className="bg-card h-8 flex-1"
          ...
          {editing && !isPlatformManaged ? (
            <label className="border-border bg-muted/40 flex items-start gap-3 rounded-md border px-4 py-3">
              <Checkbox
                checked={takeover}
              ...
              <span className="flex flex-col gap-0.5">
                ...
                <span className="text-muted-foreground text-xs leading-relaxed">
                  Convert this client-connected channel to platform-managed. The
                  client keeps enable/disable control but can no longer edit
                  credentials.
                </span>
              </span>
            </label>
          ...
          <div className="bg-primary-soft flex items-start gap-2 rounded-md px-4 py-3">
            <CheckCircle2
              className="text-primary mt-0.5 size-4 shrink-0"
              aria-hidden
            />
            <p className="text-muted-foreground text-xs leading-relaxed">
              Secrets are encrypted server-side and never shown again. New and
              updated connections start disabled — run a connection test, then
              enable.
            </p>
          </div>
        </div>
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-3 px-4 pb-4">
          <div className="border-border bg-muted/40 flex items-center justify-between gap-4 rounded-md border px-4 py-3.5">
            <div className="flex flex-col gap-0.5">
              <span className="text-foreground text-sm font-medium">
                Client can enable / disable
              </span>
              <span className="text-muted-foreground text-xs">
                Turn off to lock the toggle — e.g. while the number is under
                carrier review.
              </span>
            </div>
            <Switch
          ...
          <div className="border-border bg-muted/40 flex flex-col gap-2 rounded-md border px-4 py-3.5">
            <span className="text-foreground text-sm font-medium">
              Status notice for the client
            </span>
            <Textarea
              value={notice}
          ...
          {!canToggle && !notice.trim() ? (
            <p className="rounded-md bg-amber-500/10 px-4 py-3 text-xs leading-relaxed text-amber-700 dark:text-amber-400">
              Locking without a notice shows the client a generic &quot;contact
              support&quot; message. Adding a short reason avoids confusion and
              support tickets.
            </p>
          ) : null}
        </div>
