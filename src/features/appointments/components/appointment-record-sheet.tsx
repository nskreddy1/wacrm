...
import { RecordField, RecordLookup, RecordSection, RecordSheet } from "@/components/shared/record-sheet"
...
type FormErrors = Partial<Record<"contact" | "title" | "date" | "time", string>>
...
const APPOINTMENT_REQUIRED_FIELDS: RecordEditorField[] = [
  ...
  { id: "schedule", label: "Date & Time", typeLabel: "Date Picker", required: true },
]
 ...
 */
export function AppointmentRecordSheet({ open, onOpenChange, onCreated }: {
  open: boolean
  ...
  function validate() {
    ...
    if (!contactId) next.contact = "Select the contact attending this appointment."
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not create the appointment")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <>
      ...
      >
        <RecordSection id="appointment-information" title="Appointment Information">
          <RecordField label="Contact" htmlFor="appointment-contact" error={errors.contact}>
            <div className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <RecordLookup
                  ...
                  onSelect={(id) => { setContactId(id); setErrors((current) => ({ ...current, contact: undefined })) }}
          ...
          <RecordField label="Title" htmlFor="appointment-title" error={errors.title}>
            <Input id="appointment-title" value={title} onChange={(event) => { setTitle(event.target.value); setErrors((current) => ({ ...current, title: undefined })) }} placeholder="Consultation, follow-up, demo…" aria-invalid={Boolean(errors.title)} className="h-11" />
          </RecordField>
          {!hidden.has("service") && (
            <RecordField label="Service" htmlFor="appointment-service">
              <RecordLookup
                ...
                icon={<Tag className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />}
                onSelect={handleServiceChange}
              />
            </RecordField>
          )}
          <RecordField label="Date & Time" htmlFor="appointment-date" error={errors.date ?? errors.time}>
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="relative">
                <Input id="appointment-date" type="date" value={date} onChange={(event) => { setDate(event.target.value); setErrors((current) => ({ ...current, date: undefined })) }} aria-invalid={Boolean(errors.date)} className="h-11" />
                <CalendarClock className="pointer-events-none absolute right-9 top-1/2 hidden size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
              </div>
              <Select items={timeItems} value={startTime} onValueChange={(value) => value && setStartTime(value)}>
                <SelectTrigger aria-label="Start time" className="h-11 w-full"><SelectValue placeholder="Start time" /></SelectTrigger>
                <SelectContent><SelectGroup>{TIME_SLOTS.map((slot) => <SelectItem key={slot.value} value={slot.value}>{slot.label}</SelectItem>)}</SelectGroup></SelectContent>
              </Select>
            </div>
          </RecordField>
          <RecordField label="Duration" htmlFor="appointment-duration">
            <Select items={durationItems} value={duration} onValueChange={(value) => value && setDuration(value)}>
              <SelectTrigger id="appointment-duration" aria-label="Duration" className="h-11 w-full"><SelectValue /></SelectTrigger>
              <SelectContent><SelectGroup>{DURATION_OPTIONS.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectGroup></SelectContent>
            </Select>
          </RecordField>
          {!hidden.has("location") && (
            <RecordField label="Location" htmlFor="appointment-location">
              <Input id="appointment-location" value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Office, video call, client site…" className="h-11" />
            </RecordField>
          )}
          {!hidden.has("notes") && (
            <RecordField label="Notes" htmlFor="appointment-notes">
              <Textarea id="appointment-notes" value={notes} onChange={(event) => setNotes(event.target.value)} rows={2} placeholder="Anything the team should know beforehand" className="min-h-11 resize-none" />
            </RecordField>
          )}
        </RecordSection>

        {(layout?.custom.length ?? 0) > 0 && (
          <RecordSection id="appointment-additional" title="Additional Information">
            {layout?.custom.map((field) => (
              <RecordField key={field.id} label={field.label} htmlFor={`appointment-custom-${field.id}`}>
                <Input
      ...
      <QuickCreateContact
        ...
        onCreated={(contact) => { setExtraContacts((current) => [...current, contact]); setContactId(contact.id) }}
      />
