...
export function mapDeal(row: Row): PipelineDeal {
  ...
  return {
    ...
    customValues: (row.custom_values && typeof row.custom_values === 'object'
      ? row.custom_values
...
export function mapSubPipeline(row: Row, memberships: Row[]): SubPipeline {
  return {
    ...
    position: Number(row.position ?? 0),
    dealIds: memberships
      .filter((item) => item.sub_pipeline_id === row.id)
      .sort((a, b) => Number(a.position) - Number(b.position))
      .map((item) => String(item.deal_id)),
  };
}
