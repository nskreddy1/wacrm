...
export function PipelineSheet({
  ...
  function keyboard(event: React.KeyboardEvent, row: number, column: number) {
    ...
    const next = nextCell(row, column, deals.length, event.shiftKey);
    refs.current.get(`${next.row}:${next.column}`)?.focus();
  }
  return (
            ...
            <tr key={deal.id}>
              {columns.map((column, index) => {
                ...
                return (
                  <td
                    key={column}
                    className={`bg-background relative border-r border-b p-0 ${index === 0 ? 'sticky left-0 z-10' : ''}`}
                  >
                    {column === 'stageId' ? (
                      <Select
                        ...
                        value={deal.stageId}
                        onValueChange={(value) =>
                          value && void commit(deal, column, value)
                        }
                      >
                        <SelectTrigger
                          ref={(node) => {
                            if (node) refs.current.set(`${row}:${index}`, node);
                          }}
                          onKeyDown={(event) => keyboard(event, row, index)}
                          className="h-10 w-full rounded-none border-0"
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <Input
                        ...
                        defaultValue={String(deal[column] ?? '')}
                        type={
                          column === 'value'
                            ? 'number'
                            : column === 'due'
                        ...
                        onBlur={(event) => {
                          const value =
                            column === 'value'
                              ...
                              : event.currentTarget.value || null;
                          if (
                            String(value ?? '') !== String(deal[column] ?? '')
                          )
                            void commit(deal, column, value);
                        }}
                        className="h-10 rounded-none border-0 focus-visible:ring-inset"
                      />
                    )}
                    {status && (
                      <span
