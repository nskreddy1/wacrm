...
const DEAL_FIELD_TYPES: Record<string, string> = {
  text: 'Single Line',
  number: 'Number',
  date: 'Date Picker',
};
...
export function DealFieldsEditor({
  ...
  const initialUsed = useMemo(
    () => [
      ...
      ...DEAL_OPTIONAL_FIELDS.filter(
        (field) => !layout.hidden.includes(field.id)
      ).map((field) => field.id),
  ...
  return (
    <RecordFieldsEditor
      ...
      validateField={(draft, editingId) => {
        const clash =
          [...DEAL_REQUIRED_FIELDS, ...DEAL_OPTIONAL_FIELDS].some(
