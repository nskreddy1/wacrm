...
import {
  createSubPipelineAction,
  deleteDealsAction,
  moveDealAction,
  reorderSubPipelinesAction,
  saveDealAction,
...
import type {
  ...
  PipelineSnapshot,
  PipelineStage,
} from '@/features/pipelines/lib/domain';
...
function dueState(due: string | null) {
  ...
  const days = Math.ceil(
    (new Date(`${due}T23:59:59`).getTime() - Date.now()) / 86_400_000
  );
...
export function PipelineWorkspace({
    ...
    try {
      return JSON.parse(
        localStorage.getItem(
          `pipeline-presets:${initialSnapshot.pipeline.id}`
        ) ?? '[]'
      ) as FilterPreset[];
    } catch {
      return [];
  ...
  const [activeSubPipelineId, setActiveSubPipelineId] = useState(
    initialSubPipelineId ?? snapshot.subPipelines[0]?.id ?? snapshot.pipeline.id
  );
  ...
  const [collapsedStages, setCollapsedStages] = useState<Set<string>>(() => {
    ...
    try {
      return new Set(
        JSON.parse(
          localStorage.getItem(
            `pipeline-collapsed:${initialSnapshot.pipeline.id}`
          ) ?? '[]'
        ) as string[]
      );
    } catch {
  ...
  const activeSubPipeline = snapshot.subPipelines.find(
    (item) => item.id === activeSubPipelineId
  );
  ...
  useEffect(() => {
    function shortcuts(event: KeyboardEvent) {
      ...
      if (event.key.toLowerCase() === 'n') {
        event.preventDefault();
        setDefaultStageId(snapshot.stages[0]?.id ?? '');
        setEditing('new');
      }
    ...
    return snapshot.deals
      .filter(
        (deal) =>
          ...
          (!term ||
            `${deal.title} ${deal.contact?.name ?? ''} ${deal.company ?? ''}`
              .toLowerCase()
              .includes(term))
      )
      .sort((a, b) => {
  ...
  function changeMode(mode: PipelineMode) {
    router.replace(
      pipelinePath(snapshot.accountId, snapshot.pipeline.id, mode, {
        subPipeline: activeSubPipelineId,
        savedView: initialSavedViewId,
      })
    );
  }
  function createInStage(stageId: string) {
  ...
  function savePreset() {
    ...
    setPresets(next);
    localStorage.setItem(
      `pipeline-presets:${snapshot.pipeline.id}`,
      JSON.stringify(next)
    );
    setPresetName('');
    toast.success('Filter preset saved');
  }
  async function optimisticDeal(next: PipelineDeal) {
    return mutate(
      {
        ...snapshot,
        deals: snapshot.deals.map((deal) =>
          deal.id === next.id ? next : deal
        ),
      },
      { revalidate: false }
    );
  ...
  async function moveDeal(dealId: string, stageId: string) {
    ...
    toast.success(
      `Moved to ${snapshot.stages.find((item) => item.id === stageId)?.name ?? 'stage'}`
    );
  }
  async function deleteSelected() {
    ...
    await mutate(
      {
        ...snapshot,
        deals: snapshot.deals.filter((deal) => !selected.has(deal.id)),
      },
      { revalidate: false }
    );
  ...
  async function saveDeal(input: Parameters<typeof saveDealAction>[0]) {
    ...
    const realSubPipelineId =
      isNew && activeSubPipelineId !== snapshot.pipeline.id
        ? activeSubPipelineId
        : undefined;
      ...
      (current) => {
        ...
        return {
          ...
          deals: exists
            ...
            : [result.data, ...source.deals],
          subPipelines: realSubPipelineId
            ? source.subPipelines.map((pipeline) =>
                pipeline.id === realSubPipelineId &&
                !pipeline.dealIds.includes(result.data.id)
                  ? {
                      ...pipeline,
                      dealIds: [result.data.id, ...pipeline.dealIds],
                    }
                  : pipeline
              )
            : source.subPipelines,
  ...
  async function createSubPipeline(name: string) {
    const result = await createSubPipelineAction({
      pipelineId: snapshot.pipeline.id,
      name,
      position: snapshot.subPipelines.length,
    });
    if (!result.ok) return toast.error(result.error);
    await mutate(
      { ...snapshot, subPipelines: [...snapshot.subPipelines, result.data] },
      { revalidate: false }
    );
    setActiveSubPipelineId(result.data.id);
  }
  async function reorderSubPipelines(
    items: { id: string; name: string; dealIds: string[] }[]
  ) {
    const previous = snapshot;
    const next = items.map((item, position) => ({
      ...snapshot.subPipelines.find((pipeline) => pipeline.id === item.id)!,
      ...item,
      position,
    }));
    ...
    if (!result.ok) {
      await mutate(previous, { revalidate: false });
      toast.error(result.error);
    } else toast.success('Board order saved');
  }
  function exportDeals() {
    const ok = downloadCsv('pipeline-deals.csv', [
      ['Deal', 'Contact', 'Company', 'Value', 'Stage', 'Owner', 'Closing date'],
      ...deals.map((deal) => [
        deal.title,
        deal.contact?.name ?? '',
        deal.company ?? '',
        deal.value,
        snapshot.stages.find((item) => item.id === deal.stageId)?.name ?? '',
        deal.owner?.name ?? '',
        deal.due ?? '',
      ]),
    ]);
  ...
  return (
    <div className="bg-background flex min-h-0 flex-1 flex-col overflow-hidden">
      <header className="bg-card flex shrink-0 flex-wrap items-center gap-2 border-b px-3 py-2 lg:px-4">
        <Popover>
          <PopoverTrigger
            render={
              <Button
                variant="ghost"
                size="icon"
                className="bg-primary-soft text-primary hover:bg-primary-soft-2 hover:text-primary relative rounded-full"
          ...
          <PopoverContent align="start" className="w-80">
            <div className="flex flex-col gap-4">
              <div>
                <p className="font-medium">Filter deals</p>
                <p className="text-muted-foreground text-sm">
                  Search and narrow this board.
                </p>
              </div>
              ...
              <Select
                items={{
                  all: 'All owners',
                  ...Object.fromEntries(
                    snapshot.members.map((member) => [member.id, member.name])
                  ),
                }}
                value={owner}
              ...
              <Select
                items={{
                  all: 'All stages',
                  ...Object.fromEntries(
                    snapshot.stages.map((item) => [item.id, item.name])
                  ),
                }}
                value={stage}
        ...
        <Select
          items={Object.fromEntries(
            snapshot.pipelines.map((pipeline) => [pipeline.id, pipeline.name])
          )}
          value={snapshot.pipeline.id}
          onValueChange={(id) =>
            id && router.push(pipelinePath(snapshot.accountId, id, initialMode))
          }
        >
          <SelectTrigger className="min-w-36 rounded-full sm:w-44">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectGroup>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem value="due">Closing Date</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          <Button
          ...
          <Tabs
            value={initialMode}
            onValueChange={(value) => changeMode(value as PipelineMode)}
          >
            <TabsList>
              ...
              <TabsTrigger value="sheet" aria-label="Sheet view">
                <Table2 />
              </TabsTrigger>
            </TabsList>
          </Tabs>
          <span className="flex items-center">
            <Button
              className="rounded-l-full rounded-r-none pl-4"
              onClick={() => createInStage(snapshot.stages[0]?.id ?? '')}
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                  <Button
                    size="icon"
                    className="border-primary-foreground/25 rounded-l-none rounded-r-full border-l"
                    aria-label="Create deal in a specific stage"
                  />
                }
              >
                <ChevronDown />
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <Button
                  variant="ghost"
                  size="icon"
                  className="bg-primary-soft text-primary hover:bg-primary-soft-2 hover:text-primary rounded-full"
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={exportDeals}>
                  <Download />
                  Export CSV
                </DropdownMenuItem>
              </DropdownMenuGroup>
      ...
      {initialMode !== 'board' && (
        <div
          className="bg-card grid shrink-0 grid-cols-2 border-b sm:grid-cols-4"
          aria-label="Pipeline insights"
        >
          <Insight
            icon={CircleDollarSign}
            label="Pipeline value"
            value={money(insights.value, currency)}
          />
          <Insight
            icon={Users}
            label="Open deals"
          ...
          <Insight
            ...
            value={money(insights.forecast, currency)}
          />
          <Insight
            icon={CalendarClock}
            label="Overdue"
            value={String(insights.overdue)}
            urgent={insights.overdue > 0}
          />
        </div>
      )}

      {selected.size > 0 && (
        <div className="bg-muted flex items-center gap-2 border-b px-4 py-2 text-sm">
      ...
      {deals.length === 0 && (query || activeFilterCount > 0) ? (
        <EmptyResults
          onReset={() => {
            setQuery('');
            clearFilters();
        ...
        >
          <div className="bg-muted/20 flex min-h-0 flex-1 gap-3 overflow-x-auto p-3">
            {snapshot.stages.map((item) => (
              <StageColumn
                key={item.id}
                stage={item}
                deals={deals.filter((deal) => deal.stageId === item.id)}
                onOpen={setEditing}
                onCreate={createInStage}
                currency={currency}
                collapsed={collapsedStages.has(item.id)}
                onToggleCollapsed={toggleStageCollapsed}
              />
            ))}
          </div>
        </DndContext>
      ...
      {initialMode === 'board' && (
        <SubPipelineTabs
          pipelines={snapshot.subPipelines}
          activePipelineId={activeSubPipelineId}
          onActivate={(id) => {
            setActiveSubPipelineId(id);
            router.replace(
              pipelinePath(
                snapshot.accountId,
                snapshot.pipeline.id,
                initialMode,
                { subPipeline: id, savedView: initialSavedViewId }
              )
            );
          }}
          onCreate={(name) =>
            startTransition(() => void createSubPipeline(name))
          }
          onReorder={(items) =>
            startTransition(() => void reorderSubPipelines(items))
          }
        />
      )}
      {editing !== null && (
        <PipelineDealEditor
          key={editing === 'new' ? `new-${defaultStageId}` : editing.id}
          open
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete {selected.size} selected deal
...
function Insight({
  ...
  return (
    <div className="flex min-w-0 items-center gap-3 border-r border-b px-4 py-3 sm:border-b-0">
      <span
        className={cn(
...
function StageColumn({
  ...
  currency: string;
  collapsed: boolean;
  onToggleCollapsed: (stageId: string) => void;
}) {
  ...
  if (collapsed) {
    ...
    return (
      <section
        ref={setNodeRef}
        aria-label={`${stage.name} stage, minimized`}
        className={cn(
          'bg-muted/40 flex min-h-0 w-14 shrink-0 flex-col items-stretch overflow-hidden rounded-md border shadow-xs transition-[box-shadow,border-color,width] duration-200',
      ...
      >
        ...
        <button
          type="button"
          onClick={() => onToggleCollapsed(stage.id)}
          aria-label={`Expand ${stage.name} stage`}
          className="hover:bg-muted/70 flex min-h-0 flex-1 cursor-pointer flex-col items-center py-4 transition-colors"
        >
          <span className="text-sm font-semibold [writing-mode:vertical-rl]">
            {stage.name}
          </span>
          <span className="text-muted-foreground mt-auto text-xs [writing-mode:vertical-rl]">
            {total} <span aria-hidden="true">·</span>{' '}
            {dealCountLabel(deals.length)}
          </span>
          <span className="text-muted-foreground mt-4" aria-hidden="true">
  ...
  return (
    <section
      ref={setNodeRef}
      className={cn(
        'group/stage bg-muted/40 relative flex min-h-0 w-72 flex-1 shrink-0 flex-col overflow-hidden rounded-md border shadow-xs transition-[box-shadow,border-color] duration-200',
        'min-w-[16rem]',
        isOver && 'border-primary ring-primary/20 ring-2'
      )}
    >
      <header className="bg-card shrink-0 border-b">
        <span
          ...
          aria-hidden="true"
        />
        <div className="flex items-start justify-between gap-2 px-3 pt-2 pb-2.5">
          <div className="min-w-0">
            <h2 className="truncate text-sm font-bold">{stage.name}</h2>
            <p className="text-muted-foreground mt-0.5 text-xs">
              <span className="text-foreground/80 font-medium">{total}</span>{' '}
              <span aria-hidden="true">·</span> {dealCountLabel(deals.length)}
            </p>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <Button
                  variant="ghost"
                  size="icon-sm"
                  className="text-muted-foreground opacity-0 transition-opacity group-hover/stage:opacity-100 focus-visible:opacity-100 aria-expanded:opacity-100"
                  aria-label={`${stage.name} stage actions`}
                />
              }
            >
              <Ellipsis />
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuGroup>
                <DropdownMenuItem onClick={() => onCreate(stage.id)}>
                  <Plus />
                  Add deal
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onToggleCollapsed(stage.id)}>
                  <Minimize2 />
                  Minimize stage
                </DropdownMenuItem>
              </DropdownMenuGroup>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-2">
        <p className="bg-card/60 text-muted-foreground rounded-md border border-dashed px-3 py-2.5 text-sm">
        ...
        {deals.length === 0 && (
          <div
            className={cn(
              'flex flex-1 items-center justify-center rounded-md p-6 text-center',
              isOver && 'border-primary bg-primary/5 border border-dashed'
            )}
          >
            <p className="text-muted-foreground text-sm">This stage is empty</p>
          </div>
        )}
      </div>
      <footer className="bg-card flex shrink-0 items-center justify-between border-t px-2 py-1 opacity-0 transition-opacity group-hover/stage:opacity-100 focus-within:opacity-100">
        <Button
          variant="ghost"
          size="sm"
        ...
        <Button
          variant="ghost"
          size="icon-sm"
          className="bg-card text-muted-foreground rounded-md border shadow-xs"
          onClick={() => onToggleCollapsed(stage.id)}
          aria-label={`Minimize ${stage.name} stage`}
        >
          <Minimize2 />
        </Button>
      </footer>
...
function DealCard({
  ...
  const dueLabel = deal.due
    ? new Date(`${deal.due}T00:00:00`).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      })
    : null;
  return (
    <article
      ...
      className={cn(
        'pipeline-deal-card group bg-card cursor-grab rounded-md border p-3 shadow-xs transition-shadow hover:shadow-sm active:cursor-grabbing',
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {deal.priority !== 'normal' && (
          <span
            className={cn(
              'bg-muted text-muted-foreground shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase',
              deal.priority === 'hot' && 'bg-destructive/10 text-destructive'
            )}
          >
            {deal.priority}
          </span>
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center p-6">
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        ...
        <div>
          <h2 className="font-semibold">No deals match this view</h2>
          <p className="text-muted-foreground mt-1 text-sm">
            Try a different search or reset your filters to see all deals.
          </p>
        </div>
