...
import {
  ...
  type DealInput,
} from '@/features/pipelines/lib/validation';
...
export function PipelineDealEditor({
  open,
  deal,
  defaultStageId,
  defaultSubPipelineId,
  snapshot,
  pending,
  onOpenChange,
  onSave,
  ...
  const [subPipelineId, setSubPipelineId] = useState(
    () =>
      defaultSubPipelineId ??
      snapshot.subPipelines.find(
        (entry) => deal && entry.dealIds.includes(deal.id)
      )?.id ??
      snapshot.subPipelines[0]?.id ??
      ''
  );
  ...
  const { data: fetchedItems } = useSWR(
    open && deal?.id
      ? (['deal-items', snapshot.pipeline.id, deal.id] as const)
      : null,
  ...
  const contactOptions = useMemo(() => {
    const known = snapshot.contacts.map((contact) => ({
      id: contact.id,
      label: contact.name,
    }));
    const extras = extraContacts
      .filter(
        (extra) => !snapshot.contacts.some((contact) => contact.id === extra.id)
      )
  ...
  const companyOptions = useMemo(() => {
    ...
    for (const entry of snapshot.deals)
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (dealId) {
        ...
        if (!itemsResult.ok)
          setError(`Deal saved, but the products failed: ${itemsResult.error}`);
      }
    } catch {
  ...
  async function saveLayout(next: DealFieldLayout) {
    ...
    try {
      const result = await saveDealFieldLayoutAction(
        snapshot.pipeline.id,
        next
      );
      if (result.ok) {
        await mutateLayout(result.data, { revalidate: false });
        setFieldsOpen(false);
      }
    } finally {
  ...
  return (
    <>
      <RecordSheet
        ...
        description={
          isCreate ? 'Create a deal record' : 'Edit this deal record'
        }
        saving={busy}
        isCreate={isCreate}
      ...
      >
        ...
        >
          <RecordField
            label="Deal Name"
            htmlFor="deal-title"
            error={!draft.title.trim() && error ? error : undefined}
          >
            <Input
              id="deal-title"
              autoFocus
              ...
              aria-invalid={!draft.title.trim() && Boolean(error)}
              className="h-11"
            />
          </RecordField>
          {!hidden.has('company') && (
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Select
                items={Object.fromEntries(
                  snapshot.subPipelines.map((entry) => [entry.id, entry.name])
                )}
                value={subPipelineId}
                onValueChange={(value) => value && setSubPipelineId(value)}
              >
                <SelectTrigger
                  aria-label="Sub-pipeline"
                ...
                <SelectContent>
                  <SelectGroup>
                    {snapshot.subPipelines.map((entry) => (
              ...
              <Select
                ...
                value={draft.stageId}
                onValueChange={(value) => value && update('stageId', value)}
              >
                <SelectTrigger
                  id="deal-stage"
                  aria-label="Stage"
          ...
          <RecordField
            label={`Amount (${workspaceCurrency})`}
            htmlFor="deal-value"
          >
            <div className="relative">
              <Input
          ...
          {!hidden.has('due') && (
            <RecordField label="Closing Date" htmlFor="deal-date">
              <Input
                ...
                type="date"
                value={draft.due ?? ''}
                onChange={(event) => update('due', event.target.value || null)}
                className="h-11"
              />
            </RecordField>
          )}
          ...
          >
            <RecordField label="Priority" htmlFor="deal-priority">
              <Select
                ...
                onValueChange={(value) =>
                  value && update('priority', value as DealInput['priority'])
                }
              >
                <SelectTrigger id="deal-priority" className="h-11 w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {(['low', 'normal', 'high', 'hot'] as const).map(
                      (value) => (
            ...
            <RecordField label="Status" htmlFor="deal-status">
              <Select
                items={{ open: 'Open', won: 'Won', lost: 'Lost' }}
                value={draft.status}
                onValueChange={(value) =>
                  value && update('status', value as DealInput['status'])
                }
              >
                <SelectTrigger id="deal-status" className="h-11 w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    {(['open', 'won', 'lost'] as const).map((value) => (
                      <SelectItem key={value} value={value}>
                        {value[0].toUpperCase() + value.slice(1)}
                      </SelectItem>
                    ))}
            ...
            <RecordField label="Next Step" htmlFor="deal-next-step">
              <Input
                ...
                onChange={(event) =>
                  update('nextStep', event.target.value || null)
                }
                className="h-11"
              />
            </RecordField>
        ...
        {!hidden.has('catalog') && (
          <DealItemsTable
            ...
            onChange={(next) => {
              ...
              if (total > 0) update('value', Math.round(total * 100) / 100);
            }}
          />
        )}
