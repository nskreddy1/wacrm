...
import type { ModuleFieldLayout, ModuleKey } from "@/features/module-fields/lib/validation"
 ...
 */
export function ModuleFieldsEditor({ open, module: moduleKey, title, badge, sectionTitle, requiredFields, optionalFields, layout, onOpenChange, onSaved }: {
  open: boolean
  module: ModuleKey
  title: string
  badge?: string
  ...
  const optionalIds = useMemo(() => new Set(optionalFields.map((field) => field.id)), [optionalFields])
  ...
  const initialUsed = useMemo(() => [
    ...
    ...optionalFields.filter((field) => !layout.hidden.includes(field.id)).map((field) => field.id),
  ...
  return (
    <RecordFieldsEditor
      ...
      validateField={(draft, editingId) => {
        const clash = [...requiredFields, ...optionalFields].some((field) => field.label.toLowerCase() === draft.label.toLowerCase())
