...
export type ModuleFieldsActionResult =
  { ok: true; data: ModuleFieldLayout } | { ok: false; error: string };
...
export async function getModuleFieldLayoutAction(
  rawModule: string
): Promise<ModuleFieldsActionResult> {
  try {
    ...
    return {
      ok: true,
      data: moduleFieldLayoutSchema.parse(data?.layout ?? {}),
    };
  } catch (error) {
    return fail(error);
  }
...
export async function saveModuleFieldLayoutAction(
  rawModule: string,
  raw: unknown
): Promise<ModuleFieldsActionResult> {
  try {
