...
describe('orchestrateInboundChannelMessage', () => {
  it('stops after a deterministic Flow consumes the inbound message', async () => {
    mocks.dispatchInboundToFlows.mockResolvedValue({ consumed: true, outcome: 'started' })
  ...
  it('runs content and lifecycle automations before AI when unconsumed', async () => {
    const input = { ...baseInput, contactCreated: true, isFirstInboundMessage: true }

    await orchestrateInboundChannelMessage(input)

    const context = { message_text: 'Hello there', conversation_id: 'conversation-1' }
    expect(mocks.runAutomationsForTrigger.mock.calls.map(([call]) => call)).toEqual([
      { accountId: 'account-1', triggerType: 'new_message_received', contactId: 'contact-1', context },
      { accountId: 'account-1', triggerType: 'keyword_match', contactId: 'contact-1', context },
      { accountId: 'account-1', triggerType: 'new_contact_created', contactId: 'contact-1', context },
      { accountId: 'account-1', triggerType: 'first_inbound_message', contactId: 'contact-1', context },
    ])
    ...
    expect(mocks.runAutomationsForTrigger.mock.invocationCallOrder.at(-1)).toBeLessThan(
      mocks.dispatchInboundToAiReply.mock.invocationCallOrder[0],
    )
  })

  it('does not invoke AI for non-text or blank text messages', async () => {
    await orchestrateInboundChannelMessage({ ...baseInput, contentType: 'image' })
  ...
  it('contains downstream failures so provider acknowledgments are unaffected', async () => {
    mocks.dispatchInboundToFlows.mockRejectedValue(new Error('flow unavailable'))
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)

    await expect(orchestrateInboundChannelMessage(baseInput)).resolves.toBeUndefined()
