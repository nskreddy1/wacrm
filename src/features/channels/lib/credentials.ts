...
export function buildProviderCredentials(
  ...
  if (provider === 'smtp') {
    ...
    return {
      provider: 'smtp',
      value: { username: input.username, password: input.password },
    };
  }
  if (provider === 'resend') {
  ...
  if (provider === 'meta') {
    ...
    if (!input.accessToken)
      throw new Error('Meta permanent access token is required');
    return {
      provider: 'meta',
      value: {
  ...
  if (provider === 'twilio') {
    ...
    if (
      ...
      throw new Error(
        'Messaging Service SID must look like MG… (34 characters)'
      );
    }
    return {
      provider: 'twilio',
