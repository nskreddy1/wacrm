...
async function twilioGet(
  ...
  if (response.status === 401) {
    throw new DiscoveryError(
      'Twilio rejected these credentials. Check the Account SID and Auth token.',
      401
    );
  }
  if (!response.ok) {
...
export async function discoverTwilioAccount(
  accountSid: string,
  authToken: string
): Promise<TwilioDiscovery> {
  if (!/^AC[0-9a-fA-F]{32}$/.test(accountSid)) {
    throw new DiscoveryError(
      'Account SID must look like AC… (34 characters).',
      400
    );
  }
  ...
  try {
    ...
    const senders = Array.isArray(sendersPayload.senders)
      ? (sendersPayload.senders as Record<string, unknown>[])
      : [];
