...
import { phoneVariants } from '@/features/whatsapp/lib/phone-utils';
...
function resolveSendContext(
  ...
  if (credentials.provider !== 'meta')
    throw new Error('Meta credentials required');
  const phoneNumberId =
...
async function postMetaMessage(
  ...
  const response = await fetch(
    `${META_API_BASE}/${ctx.phoneNumberId}/messages`,
    {
      method: 'POST',
      headers: {
  ...
  if (!response.ok || !data.messages?.[0]?.id) {
    throw new Error(
      data.error?.message ??
        `Meta API error: ${response.status} (code ${data.error?.code ?? 'unknown'})`
    );
  }
...
export class MetaWhatsAppAdapter implements ChannelAdapter {
  ...
  async send(message: OutboundChannelMessage): Promise<ChannelSendResult> {
    ...
    const candidates =
      variants.length > 0 ? variants : [message.recipient.identity];
  ...
  async checkHealth(connection: ChannelConnection): Promise<ChannelHealth> {
    try {
      ...
      return {
        ...
        error:
          error instanceof Error ? error.message : 'Meta health check failed',
      };
    }
  }
}
