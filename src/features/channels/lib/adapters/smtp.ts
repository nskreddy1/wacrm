...
function transportFor(connection: ChannelConnection) {
  ...
  if (configuration.secure && configuration.port === 587) {
    throw new Error('Port 587 uses STARTTLS; turn off implicit TLS and enable Require TLS.')
  }
  const credentials = decryptProviderCredentials(connection)
  if (credentials.provider !== 'smtp') throw new Error('SMTP credentials required')
  return nodemailer.createTransport({
    host: configuration.host,
    port: configuration.port,
    secure: configuration.secure,
    requireTLS: configuration.requireTls,
    auth: { user: credentials.value.username, pass: credentials.value.password },
...
export class SmtpEmailAdapter implements ChannelAdapter {
  ...
  readonly capabilities = { send: true, receive: false, healthCheck: true, oauth: false, testMessage: true } as const
  ...
  async send(message: OutboundChannelMessage): Promise<ChannelSendResult> {
    ...
    const configuration = smtpConfigurationSchema.parse(message.connection.configuration)
    const info = await transportFor(message.connection).sendMail({
      from: configuration.fromName ? { name: configuration.fromName, address: sender } : sender,
    ...
    return { externalMessageId: info.messageId, acceptedAt: new Date().toISOString() }
  }
  ...
  async checkHealth(connection: ChannelConnection): Promise<ChannelHealth> {
    try {
      ...
      return { ok: false, checkedAt: new Date().toISOString(), error: safeSmtpError(error) }
    }
  }

  async sendTest(connection: ChannelConnection, recipient: string): Promise<ChannelSendResult> {
    return this.send({
