...
import type { TwilioCredentials } from '@/features/channels/lib/credentials';
...
async function request<T>(
  ...
  if (!response.ok)
    throw new Error(
      result.message ?? `Twilio Content API failed (${response.status})`
    );
  return result;
}

export async function listTwilioContent(
  credentials: TwilioCredentials
  ...
  while (url) {
    ...
    url = next
      ? next.startsWith('http')
        ? next
        : `https://content.twilio.com${next}`
      : null;
  }
  return items;
}

export async function getTwilioApproval(
  credentials: TwilioCredentials,
  sid: string
): Promise<TwilioApproval | null> {
  try {
...
export async function listTwilioContentAndApprovals(
  ...
  let url: string | null =
    `https://content.twilio.com/v2/ContentAndApprovals?PageSize=100`;
  while (url) {
    const page: {
      contents?: TwilioContentWithApproval[];
      meta?: { next_page_url?: string | null };
    } = await request(credentials, url);
    items.push(...(page.contents ?? []));
    const next: string | null | undefined = page.meta?.next_page_url;
...
export async function createTwilioContent(
  ...
  const quickReplies = (input.buttons ?? [])
    ...
    .map((button) => ({ title: button.text, id: button.text }));
  const actions: Array<Record<string, string>> = [];
  for (const button of input.buttons ?? []) {
    if (button.type === 'URL')
      actions.push({ type: 'URL', title: button.text, url: button.url });
...
export async function submitTwilioApproval(
  credentials: TwilioCredentials,
  sid: string,
  input: { name: string; category: string }
) {
  return request<TwilioApproval>(
    credentials,
    `${CONTENT_BASE}/Content/${encodeURIComponent(sid)}/ApprovalRequests/whatsapp`,
    {
      method: 'POST',
      body: JSON.stringify({
        name: input.name,
...
export function normalizeTwilioContent(item: TwilioContentItem) {
  ...
  for (const action of actions) {
    if (action.type === 'URL')
      buttons.push({
        type: 'URL',
        text: action.title ?? 'Open',
        url: action.url ?? '',
      });
    else if (action.type === 'PHONE')
      buttons.push({
        type: 'PHONE_NUMBER',
        text: action.title ?? 'Call',
