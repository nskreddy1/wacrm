...
export type ConnectMethod =
  'twilio-popup' | 'twilio-manual' | 'twilio-reuse' | 'meta-cloud';
  ...
  return (
    <DialogContent className="sm:max-w-lg">
      <DialogHeader>
        ...
        <DialogDescription>
          Choose how you want to connect. With one-click sign-in, a popup asks
          you to authorize your provider account — we never see your password or
          auth token.
        </DialogDescription>
      </DialogHeader>
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          ...
          <Select
            ...
            onValueChange={(value) => {
              if (value) setMethod(value as ConnectMethod);
            }}
          >
            ...
            <SelectContent>
              <SelectGroup>
                {authorizeUrl ? (
                  <SelectItem value="twilio-popup">
                    Twilio — one-click sign in (recommended)
                  </SelectItem>
                ) : null}
                {reusable ? (
                  <SelectItem value="twilio-reuse">
                    Use existing Twilio account — &ldquo;{reusable.displayName}
                    &rdquo; ({reusable.channelLabel})
                  </SelectItem>
                ) : null}
                <SelectItem value="twilio-manual">
                  Twilio — enter credentials manually
                </SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          {method === 'twilio-reuse' && reusable ? (
            <p className="text-muted-foreground text-xs leading-relaxed">
              The Account SID and Auth token stored for {reusable.channelLabel}{' '}
              are copied server-side — no retyping, and secrets never reach this
              browser.
            </p>
          ) : null}
        </div>
        {method === 'twilio-popup' ? (
          <Alert className="border-amber-500/30 bg-amber-500/10">
            <AlertDescription className="text-foreground/85">
              Make sure popups are allowed in your browser settings. Otherwise
              you can&apos;t complete the sign-in process to link your {label}{' '}
              account.
            </AlertDescription>
          </Alert>
        ) : null}
        <div className="flex items-start gap-3">
          ...
          />
          <Label
            htmlFor="connect-terms"
            className="text-foreground/85 text-sm leading-relaxed font-normal"
          >
            I authorize this workspace to access my provider account for sending
            and receiving {label} messages, and I have read the provider&apos;s
            messaging policies.
          </Label>
        </div>
      </div>
