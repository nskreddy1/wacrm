  ...
  if (mode === 'auto_reply') {
    parts.push(
      ...
      `After your reply (including after the ${HANDOFF_SENTINEL} line), end your output with exactly one final line in this exact format and nothing after it:\n` +
        `${META_SENTINEL}{"sentiment":"angry|frustrated|neutral|happy","escalate":true|false,"reason":"human_requested|angry_customer|out_of_scope|needs_account_data|purchase_ready|none","language":"<tag>","emotions":{"<label>":0.7}}\n` +
        'Pick the single sentiment that best matches the customer\'s latest messages. Set "escalate" to true whenever a human should take over (same conditions as the handoff rule, plus a customer ready to buy who needs a person, or a request needing their account data). When "escalate" is false, use "reason":"none". For "language", give the customer\'s language as a lowercase tag with a "-latn" suffix when they write a non-Latin-script language romanized: "en" English, "hi" Hindi in Devanagari, "hi-latn" romanized Hinglish, "ta" Tamil, "ta-latn" romanized Tamil, "kn" Kannada, "ml" Malayalam, "te" Telugu, "bn" Bengali, "mr" Marathi, and so on for any other language. For "emotions", include ONLY the emotions actually present in the customer\'s recent messages, each scored 0 to 1 by intensity, chosen from exactly: "anger", "frustration", "disappointment", "confusion", "anxiety", "urgency", "satisfaction", "gratitude" — multiple may apply at once (a customer can be angry AND still hopeful); use {} when none apply. Judge emotion from meaning, not keywords — it works the same in any language or script, including romanized text like "yaar order kab aayega??". This metadata line is machine-read and stripped before sending — the customer never sees it.'
    );
  }
