    ...
    >
      {editing && (
        <div className="border-border bg-card absolute -top-2.5 right-2 z-10 flex items-center gap-1 rounded-full border px-1 py-0.5 shadow-sm">
          <button
            type="button"
            className="text-muted-foreground hover:bg-muted hover:text-foreground flex size-6 cursor-grab items-center justify-center rounded-full transition-colors active:cursor-grabbing"
      ...
      <div
        className={cn('h-full', editing && 'pointer-events-none select-none')}
      >
  ...
  function cycleSize(id: string) {
    persist(
      widgets.map((w) => {
        ...
        const next =
          WIDGET_SIZES[
            (WIDGET_SIZES.indexOf(w.size) + 1) % WIDGET_SIZES.length
          ];
        return { ...w, size: next };
      })
    );
      ...
      <>
        <div className="border-border flex min-h-[50vh] flex-col items-center justify-center gap-3 rounded-xl border border-dashed p-8 text-center">
          ...
          <p className="text-muted-foreground max-w-sm text-xs leading-relaxed text-pretty">
            Add KPI cards, charts, target meters and panels to build a view that
