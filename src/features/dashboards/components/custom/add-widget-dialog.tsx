    ...
    >
      <DialogContent className="flex max-h-[min(640px,calc(100dvh-3rem))] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
        ...
        <div className="grid flex-1 overflow-y-auto md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
          {/* Live preview pane */}
          <div className="border-border bg-muted/30 flex flex-col gap-3 border-b p-5 md:border-r md:border-b-0">
            <p className="text-muted-foreground text-xs font-medium tracking-wide uppercase">
              Preview
            </p>
            <div
              className="pointer-events-none min-h-[220px] flex-1 [&>*]:h-full"
              aria-hidden="true"
            >
              <WidgetRenderer
                widget={draft}
          ...
          <div className="flex flex-col gap-4 p-6">
            ...
            {type === 'kpi' && (
              <FormRow
                label="Measure"
                htmlFor="widget-kpi-metric"
                hint={KPI_METRICS[kpiMetric].description}
              >
                <Select
                  value={kpiMetric}
                  onValueChange={(v) => setKpiMetric(v as KpiMetric)}
                >
            ...
            {type === 'target' && (
              <>
                <FormRow label="Measure" htmlFor="widget-target-metric">
                  <Select
                    value={targetMetric}
                    onValueChange={(v) => setTargetMetric(v as TargetMetric)}
                  >
                    <SelectTrigger id="widget-target-metric" className="w-full">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {(Object.keys(TARGET_METRICS) as TargetMetric[]).map(
                        (m) => (
                          <SelectItem key={m} value={m}>
            ...
            {type === 'panel' && (
              <FormRow
                label="Panel"
                htmlFor="widget-panel-kind"
                hint={PANEL_KINDS[panel].description}
              >
                <Select
                  value={panel}
                  onValueChange={(v) => setPanel(v as PanelKind)}
                >
        ...
        {/* Footer action bar */}
        <div className="border-border bg-muted/30 flex items-center justify-end gap-2 border-t px-6 py-3.5">
