...
function TargetMeter({
  ...
  return (
    <div className="border-border bg-card flex h-full flex-col rounded-xl border p-4 shadow-(--shadow-pipeline-card)">
      <div className="flex items-center justify-between gap-2">
        <div className="bg-primary-soft text-primary flex size-8 items-center justify-center rounded-lg">
          <Target className="size-4" aria-hidden="true" />
        </div>
        {reached && (
          <span className="bg-positive/10 text-positive rounded-full px-1.5 py-0.5 text-[11px] font-semibold">
            Goal reached
          </span>
        )}
      </div>
      <div className="mt-2 flex flex-1 flex-col items-center justify-center">
        <svg
          ...
          role="img"
          aria-label={`${title}: ${fmt(value)} of ${fmt(goal)} goal`}
        >
          <path
          ...
          <path
            ...
            stroke={
              reached
                ? 'var(--color-positive, var(--color-primary))'
                : 'var(--color-primary)'
            }
            strokeWidth="13"
            strokeLinecap="round"
    ...
    <div className="flex flex-col gap-3">
      {channels.map((ch) => {
        ...
        return (
          <div
            key={ch.channel}
            className="border-border bg-card-2 flex flex-col gap-2 rounded-lg border p-3"
          >
            <div className="flex items-center justify-between gap-2">
