...
export function ChartCard({
  ...
  return (
    <article
      className={cn(
        'border-border bg-card flex flex-col overflow-hidden rounded-xl border shadow-(--shadow-pipeline-card)',
        className
      )}
    >
      <header className="border-border flex items-start justify-between gap-3 border-b px-4 py-3.5 sm:px-5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {href && (
            <Link
              href={href}
              className="text-primary hover:bg-primary-soft flex items-center gap-1 rounded-md px-1.5 py-1 text-xs font-semibold transition-colors"
            >
              {hrefLabel}
...
export function CardMetaChip({ children }: { children: ReactNode }) {
  return (
    <span className="border-border bg-card-2 text-muted-foreground rounded-full border px-2 py-0.5 text-[11px] font-medium">
      {children}
    </span>
  );
}
