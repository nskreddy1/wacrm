...
function DashboardSkeleton() {
  return (
    <div
      className="mx-auto flex max-w-[1500px] flex-col gap-5 p-4 sm:p-6 lg:p-8"
      aria-busy="true"
...
export function DashboardWorkspace() {
  ...
  const firstName =
    personDisplayName(profile?.full_name, profile?.email).split(/\s+/)[0] ||
    'there';
      ...
      <div className="flex h-full items-center justify-center p-8">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-muted-foreground max-w-sm text-xs text-pretty">
            Check your connection and try again. If the problem persists, your
      ...
      <div className="mx-auto flex max-w-[1500px] flex-col gap-5 p-4 sm:p-6 lg:p-8">
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm text-pretty">{`Here is how ${workspaceName} is performing right now.`}</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="border-border bg-card text-muted-foreground mr-1 flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium">
              <span
                className="bg-positive size-1.5 rounded-full motion-safe:animate-pulse"
                aria-hidden="true"
              />
              Live
            </span>
            <Link
              href="/contacts"
              className="border-border bg-card hover:bg-muted flex h-9 items-center gap-2 rounded-lg border px-3 text-sm font-medium transition-colors"
            >
              <Plus className="size-4" aria-hidden="true" /> Add contact
            </Link>
            <Link
              href="/broadcasts/new"
              className="bg-primary text-primary-foreground flex h-9 items-center gap-2 rounded-lg px-3 text-sm font-semibold transition-opacity hover:opacity-90"
            >
          ...
          <div className="flex min-w-0 flex-col gap-5">
            ...
            <Section index={2} className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
              ...
              <ChartCard
                ...
                caption="Message share by channel, last 7 days"
                contentClassName="flex flex-col gap-4"
              >
                {channels.length === 0 && (
                  <p className="text-muted-foreground py-6 text-center text-xs">
                    No channel activity in the last 7 days.
                  </p>
                )}
                {channels.map((ch) => {
                  const total =
                    channels.reduce((sum, c) => sum + c.messages7d, 0) || 1;
                  const share = Math.round((ch.messages7d / total) * 100);
                  return (
                    <div
                      key={ch.channel}
                      className="border-border bg-card-2 flex flex-col gap-2.5 rounded-lg border p-3.5"
                    >
                      ...
                      <div className="bg-muted h-1.5 w-full overflow-hidden rounded-full">
                        <div
                          ...
                          style={{
                            width: `${share}%`,
                            background: channelColorVar(ch.channel),
                          }}
                        />
                      </div>
            ...
            <Section index={4} className="grid gap-4 lg:grid-cols-[1fr_1.6fr]">
              <ChartCard
                ...
                caption="Open vs resolved conversations per agent"
                href="/settings"
                hrefLabel="Manage"
              >
                {team.length === 0 ? (
