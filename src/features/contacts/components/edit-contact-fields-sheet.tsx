...
import {
  isReservedContactField,
  validateFieldDefinition,
} from '@/lib/data/contacts/validation';
...
export function EditContactFieldsSheet({
  ...
  const editorFields = useMemo<RecordEditorField[]>(
    () =>
      fields.map((field) => {
  ...
  const initialUsed = useMemo(() => {
    ...
    const ordered = preferences.order.filter(
      (id) => preferences.visible.includes(id) && ids.has(id)
    );
    const missing = preferences.visible.filter(
      (id) => !ordered.includes(id) && ids.has(id)
    );
    ...
    <RecordFieldsEditor
      ...
      onSave={async (usedIds) => {
        try {
          const order = [
            ...usedIds,
            ...preferences.order.filter((id) => !usedIds.includes(id)),
          ];
          await request('PATCH', {
            kind: 'preferences',
            preferences: { visible: usedIds, order },
          });
