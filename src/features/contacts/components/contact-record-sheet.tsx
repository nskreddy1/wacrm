...
} from '@/features/contacts/components/international-phone-input';
...
export function ContactRecordSheet({
  state,
  fields,
  preferences,
  owners = [],
  ...
  const [phoneType, setPhoneType] =
    useState<(typeof PHONE_TYPES)[number]>('Mobile');
  ...
  const customFields = useMemo(
    () =>
      fields.filter(
        (field) =>
          field.custom &&
          !RESERVED_FIELD_LABELS.has(field.label.toLocaleLowerCase())
      ),
    [fields]
  );
  ...
  useEffect(() => {
    ...
    const nextValues = Object.fromEntries(
      fields.map((field) => [field.id, contact?.values[field.id] ?? ''])
    );
  ...
  const displayName =
    [values.firstName, values.lastName].map(String).filter(Boolean).join(' ') ||
    String(contact?.values.name ?? 'New contact');
  const selectedOwner =
    owners.find((owner) => owner.userId === ownerId) ?? null;
  ...
  function validate() {
    ...
    if (!String(values.firstName ?? '').trim())
      next.firstName = 'Enter the contact’s first name.';
    if (!String(values.lastName ?? '').trim())
      next.lastName = 'Enter the contact’s last name.';
    if (
    ...
    if (phone && !validInternationalPhone(phone))
      next.phone = 'Choose a country and enter a valid phone number.';
    extraPhones.forEach((extra, index) => {
      if (extra.trim() && !validInternationalPhone(extra.trim()))
        next[`extraPhone-${index}`] = 'Enter a valid phone number.';
    });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/v1/workspace/contacts', {
        ...
        body: JSON.stringify(
          (() => {
            ...
            return contact
              ? { id: contact.id, values: payloadValues }
      ...
      if (!response.ok)
        throw new Error(payload.error?.message ?? 'Unable to save contact');
    ...
    if (field.type === 'checkbox') {
      ...
      control = (
        <button
          ...
          className="border-input bg-background data-[checked=true]:border-primary data-[checked=true]:bg-primary data-[checked=true]:text-primary-foreground flex size-5 items-center justify-center rounded border transition-colors disabled:pointer-events-none disabled:opacity-50"
          ...
          <DropdownMenuTrigger
            render={
              <button
                ...
                id={inputId}
                disabled={readonly}
                className="border-input hover:bg-muted flex h-11 w-full items-center justify-between rounded-lg border bg-transparent px-2.5 text-left text-sm transition-colors disabled:pointer-events-none disabled:opacity-50"
              />
            }
          >
    ...
    return (
      ...
      >
        <FieldLabel
          id={`${inputId}-label`}
          htmlFor={inputId}
          className="sm:w-36 sm:justify-end"
        >
          {field.label}
        </FieldLabel>
        {control}
            ...
            <div className="min-w-0 flex-1">
              ...
              <SheetDescription className="sr-only">
                {mode === 'create'
                  ...
                  : 'View or edit this contact record'}
              </SheetDescription>
            </div>
            {readonly ? (
              <Button
          ...
          <ScrollArea className="min-h-0 flex-1">
            <div className="flex flex-col gap-8 px-8 py-6">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        render={
                          <button
                            type="button"
                            aria-labelledby="contact-owner-label"
                            disabled={readonly}
                            className="bg-card hover:bg-muted inline-flex items-center gap-2 rounded-full border px-3 py-1.5 font-medium transition-colors disabled:pointer-events-none disabled:opacity-70"
                          />
                        }
                      >
                        <span className="bg-primary/15 text-primary flex size-6 items-center justify-center rounded-full text-xs font-semibold">
                          {ownerInitials(selectedOwner?.name ?? '?')}
                        </span>
                      ...
                      <DropdownMenuContent align="end" className="w-64">
                        {owners.length === 0 ? (
                          ...
                          owners.map((owner) => (
                            <DropdownMenuItem
                              key={owner.userId}
                              onClick={() => setOwnerId(owner.userId)}
                            >
                              <span className="bg-primary/15 text-primary flex size-6 items-center justify-center rounded-full text-xs font-semibold">
                ...
                <FieldGroup className="gap-5">
                  ...
                  <Field className="grid grid-cols-1 items-center gap-2 sm:grid-cols-[9rem_minmax(0,1fr)] sm:gap-4">
                    ...
                    <div className="relative flex-1">
                      ...
                      />
                      <Building2 className="text-muted-foreground pointer-events-none absolute top-1/2 right-3 size-5 -translate-y-1/2" />
                    </div>
                  </Field>
                  <Field
                    className="grid grid-cols-1 items-center gap-2 sm:grid-cols-[9rem_minmax(0,1fr)] sm:gap-4"
                        ...
                        <DropdownMenuTrigger
                          render={
                            <button
                              ...
                              disabled={readonly}
                              className="hover:bg-muted inline-flex items-center gap-1 rounded-md px-1 py-0.5 text-sm font-medium transition-colors disabled:pointer-events-none"
                            />
                          }
                        >
                          {phoneType}{' '}
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        {!readonly ? (
                          <Button
                            ...
                            onClick={() =>
                              setExtraPhones((current) => [...current, ''])
                            }
                          >
                        ...
                        <div key={index} className="flex items-start gap-2">
                          ...
                          {!readonly ? (
                            <Button
                              ...
                              size="icon"
                              className="text-muted-foreground mt-1 shrink-0"
                              aria-label={`Remove phone number ${index + 2}`}
                              onClick={() =>
                                setExtraPhones((current) =>
              ...
              <Collapsible open={addressOpen} onOpenChange={setAddressOpen}>
                <CollapsibleTrigger className="group focus-visible:ring-ring flex w-full items-center justify-between rounded-md py-2 text-left font-semibold focus-visible:ring-2 focus-visible:outline-none active:scale-[0.99] motion-safe:transition-transform motion-safe:duration-150">
                  <span>Address Information</span>
                  <ChevronDown className="group-data-panel-open:rotate-180 motion-safe:transition-transform motion-safe:duration-200" />
                </CollapsibleTrigger>
                <CollapsibleContent className="data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 motion-safe:transition-[height,opacity] motion-safe:duration-200">
