...
import {
  ...
  summarizeRule,
  type FilterGroup,
} from '@/lib/data/contacts/filters';
...
export function ContactWorkspace({
  ...
  const { data, error, isLoading, mutate } = useSWR<{ data: Store }>(
    `/api/v1/workspace/contacts?view=${encodeURIComponent(savedViewId)}&mode=${initialView}`
  );
  ...
  const detail = !detailDismissed
    ? (store?.contacts.find((contact) => contact.id === initialContactId) ??
      null)
    : null;
  ...
  const [customVisibleFieldIds, setVisibleFieldIds] = useState<string[] | null>(
    null
  );
  const visibleFieldIds =
    customVisibleFieldIds ?? store?.preferences.visible ?? [];
  ...
  const orderedFields = useMemo(() => {
    ...
    return store.preferences.order
      .map((id) => byId.get(id))
      .filter(Boolean) as ContactField[];
  }, [store]);
  ...
  const filtered = useMemo(() => {
    ...
    const searched = term
      ? store.contacts.filter((contact) =>
          Object.values(contact.values).some((value) =>
            valueText(value).toLowerCase().includes(term)
          )
        )
      : [...store.contacts];
    const rows = searched.filter((contact) =>
      matchesFilter(contact, store.fields, filters)
    );
    if (sort)
      rows.sort(
        (a, b) =>
          valueText(a.values[sort.field]).localeCompare(
            valueText(b.values[sort.field]),
            undefined,
            { numeric: true }
  ...
  async function saveCell(
    ...
    try {
      await api('PATCH', { id: contact.id, values: { [field.id]: value } });
      await mutate();
    } catch (saveError) {
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      await mutate();
      toast.success(
        `${deletedCount} ${deletedCount === 1 ? 'contact' : 'contacts'} deleted`
      );
    } catch (deleteError) {
      toast.error(
  ...
  function exportCurrentResults() {
    const exported = downloadCsv('contacts-current-results.csv', [
      visibleFields.map((field) => field.label),
      ...filtered.map((contact) =>
        visibleFields.map((field) => valueText(contact.values[field.id]))
      ),
    ]);
    if (exported)
      toast.success(
        `Exported ${filtered.length} ${filtered.length === 1 ? 'contact' : 'contacts'}`
      );
  }
  async function setVisible(fieldId: string, checked: boolean) {
    const previous = visibleFieldIds;
    const visible = checked
      ? Array.from(new Set([...visibleFieldIds, fieldId]))
    ...
    try {
      ...
      toast.error(
        preferenceError instanceof Error
          ...
          : 'Unable to update displayed columns'
      );
    }
  }
  ...
  if (error)
    return (
      <div className="flex min-h-[60vh] items-center justify-center p-6">
        <FeatureState
          icon={RefreshCw}
          title="Contact workspace unavailable"
          description="We couldn't securely load this account's contacts. No records were changed; retry the request to reconnect."
  ...
  return (
    <div className="bg-background flex min-h-0 flex-1 flex-col">
      <div className="bg-card flex flex-wrap items-center gap-2 border-b px-3 py-2">
        ...
        >
          <TabsList aria-label="Contact view">
            <TabsTrigger value="list" aria-label="List view" title="List view">
              <List />
            </TabsTrigger>
            <TabsTrigger
              value="sheet"
              aria-label="Editable sheet view"
              title="Editable sheet view"
            >
              <SheetIcon />
            </TabsTrigger>
            <TabsTrigger value="cards" aria-label="Card view" title="Card view">
              <Grid2X2 />
            </TabsTrigger>
          </TabsList>
        </Tabs>
        <Popover>
          <PopoverTrigger
            render={
              <Button
                variant="outline"
                size="sm"
                aria-label="Choose displayed columns"
              />
            }
          >
          ...
          <PopoverContent align="end" className="w-80 p-0">
            <div className="flex items-center justify-between p-3">
              <div>
                <p className="font-medium">Displayed columns</p>
                <p className="text-muted-foreground text-xs">
                  Personal display preference · {visibleFields.length} of{' '}
              ...
              <Button
                variant="ghost"
                size="sm"
                disabled={visibleFields.length === orderedFields.length}
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setVisible(field.id, Boolean(checked))
                      }
                    />
            ...
            <p className="text-muted-foreground p-3 text-xs">
              Name is required and always displayed. Manage the field schema
              from More actions.
            </p>
          </PopoverContent>
        </Popover>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64">
            <DropdownMenuGroup>
              ...
              <DropdownMenuItem
                disabled={filtered.length === 0 || visibleFields.length === 0}
                onClick={exportCurrentResults}
              >
                <Download /> Export current results
            ...
            <DropdownMenuGroup>
              ...
              <DropdownMenuItem onClick={() => setFieldsManagerOpen(true)}>
                <SlidersHorizontal /> Manage custom fields
              </DropdownMenuItem>
            </DropdownMenuGroup>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      {countRules(filters) > 0 && (
        <div className="bg-muted/40 flex flex-wrap items-center gap-2 border-b px-3 py-2">
          ...
          {flattenRules(filters).map((rule) => (
            <Badge
              key={rule.id}
              variant="outline"
              className="bg-background gap-1"
            >
              {summarizeRule(rule, store.fields)}
            </Badge>
          ))}
      ...
      <div className="min-h-0 flex-1 overflow-auto">
              ...
              <tr>
                ...
                {visibleFields.map((field) => (
                  <th
                    ...
                    style={{
                      minWidth:
                        store.preferences.widths[field.id] ?? field.width,
                    }}
                    className={sheetTable.th}
                  >
                    <button
            ...
            <tbody>
              {rows.map((contact) => (
                ...
                >
                  ...
                  {visibleFields.map((field) => {
                    const active =
                      editing?.id === contact.id && editing.field === field.id;
                    const saveKey = `${contact.id}:${field.id}`;
                    return (
                      <td
                        key={field.id}
                        className={sheetTable.tdFlush}
                        onDoubleClick={() =>
                          view === 'sheet' &&
                          !field.readOnly &&
                          setEditing({ id: contact.id, field: field.id })
                        }
                      >
                        {active ? (
                          field.type === 'single_select' ? (
                            <select
                              autoFocus
                              defaultValue={valueText(contact.values[field.id])}
                              onBlur={(event) =>
                                saveCell(contact, field, event.target.value)
                              }
                              onChange={(event) =>
                                saveCell(contact, field, event.target.value)
                              }
                              className="bg-background h-10 w-full px-3 outline-none"
                            >
                            ...
                            <Input
                              ...
                              className="h-10 rounded-none border-0 ring-inset"
                            />
                          )
                        ) : (
                          <button
                            className={cn(
                              ...
                              view === 'sheet' && 'cursor-cell',
                              field.id === 'name' && 'text-primary font-medium'
                            )}
                            onClick={() =>
                              view === 'list'
                                ? openContact(contact)
                          ...
                          >
                            {saving === saveKey
                              ? 'Saving…'
                              : field.type === 'currency'
                                ? `$${Number(contact.values[field.id] ?? 0).toLocaleString()}`
                                : valueText(contact.values[field.id]) || '—'}
                          </button>
                        )}
                      </td>
                    );
        ...
        {view === 'cards' && (
          <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
            {rows.map((contact) => (
              <article
                key={contact.id}
                className="bg-card flex flex-col gap-4 rounded-xl border p-4 shadow-xs"
              >
                <div className="flex items-start gap-3">
                  ...
                  <Button
                    ...
                    size="icon-sm"
                    title="Edit contact"
                    aria-label={`Edit ${valueText(contact.values.name)}`}
                    onClick={() => setContactSheet({ mode: 'edit', contact })}
                  >
                    <Pencil />
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="truncate">
                      {valueText(contact.values.email)}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Phone</p>
                    <p>{valueText(contact.values.phone)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Location</p>
                    <p>{valueText(contact.values.city)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Value</p>
                    <p>${Number(contact.values.value ?? 0).toLocaleString()}</p>
                  </div>
                </div>
              </article>
            ))}
        ...
        {filtered.length === 0 && (
          <div className="flex min-h-80 items-center justify-center p-6">
            <FeatureState
              icon={hasRefinements ? Search : Users}
              title={hasRefinements ? 'No contacts match' : 'No contacts yet'}
              description={
                hasRefinements
                  ? 'Clear search or filters to broaden these results.'
                  : 'Create a contact now, or import a CSV when you have an existing list.'
              }
              action={{
                ...
                onClick: () => {
                  if (hasRefinements) {
                    ...
                    setFilters(emptyFilterGroup());
                    setPage(0);
                  } else setContactSheet({ mode: 'create' });
                },
              }}
            />
          </div>
      ...
      <footer className="bg-card flex min-h-14 flex-wrap items-center gap-x-4 gap-y-2 border-t border-b px-4 py-2 text-xs shadow-xs">
        <span aria-live="polite">
          {hasRefinements ? 'Matching contacts' : 'Total contacts'}{' '}
          <strong>{filtered.length}</strong>
        </span>
        {showPagination && (
          <>
            <label className="ml-auto" htmlFor="contacts-page-size">
              Rows per page
            </label>
            <Select
      ...
      <ContactRecordSheet
        state={
          contactSheet ?? (detail ? { mode: 'view', contact: detail } : null)
        }
        fields={orderedFields}
          ...
          if (!open) {
            ...
            if (window.location.search.includes('contact='))
              window.history.replaceState(
                null,
                '',
                contactsPath(undefined, { mode: view, view: savedViewId })
              );
          }
        }}
        onSaved={() => mutate()}
