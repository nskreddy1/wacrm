...
import { isGroup, operatorsFor, validateFilterGroup, type FilterGroup, type FilterOperator, type FilterRule } from "@/lib/data/contacts/filters"
...
function Rule({ rule, fields, root, change }: { rule: FilterRule; fields: ContactField[]; root: FilterGroup; change: (value: FilterGroup) => void }) {
  ...
  return <div className="grid grid-cols-[1fr_1fr_auto] gap-2 rounded-lg border bg-background p-2 sm:grid-cols-[1fr_1fr_1fr_auto]">
    <Select value={rule.field} onValueChange={(value) => { if (!value) return; const next = fields.find((item) => item.id === value); patch({ field: value, operator: operatorsFor(next)[0]?.value ?? "is", value: "" }) }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{fields.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectGroup></SelectContent></Select>
    <Select value={rule.operator} onValueChange={(value) => value && patch({ operator: value as FilterOperator })}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{operatorsFor(field).map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectGroup></SelectContent></Select>
    {!rule.operator.includes("empty") && (field?.type === "single_select" || field?.type === "checkbox" ? <Select value={rule.value} onValueChange={(value) => value !== null && patch({ value })}><SelectTrigger className="col-span-2 w-full sm:col-span-1"><SelectValue placeholder="Select value" /></SelectTrigger><SelectContent><SelectGroup>{(field.type === "checkbox" ? ["true", "false"] : field.options ?? []).map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectGroup></SelectContent></Select> : <Input className="col-span-2 sm:col-span-1" type={field?.type === "date" ? "date" : ["number", "currency"].includes(field?.type ?? "") ? "number" : "text"} value={rule.value} onChange={(event) => patch({ value: event.target.value })} placeholder="Value" />)}
...
export function ContactFilterBuilder({ open, fields, value, onOpenChange, onApply, onClear }: { open: boolean; fields: ContactField[]; value: FilterGroup; onOpenChange: (open: boolean) => void; onApply: (value: FilterGroup) => void; onClear: () => void }) {
  // Key-based remount: the body initializes its draft from `value` in a
  // useState initializer and remounts fresh per open cycle — no sync effect.
  return <Sheet open={open} onOpenChange={onOpenChange}>{open && <FilterBuilderBody fields={fields} value={value} onOpenChange={onOpenChange} onApply={onApply} onClear={onClear} />}</Sheet>
}
function FilterBuilderBody({ fields, value, onOpenChange, onApply, onClear }: { fields: ContactField[]; value: FilterGroup; onOpenChange: (open: boolean) => void; onApply: (value: FilterGroup) => void; onClear: () => void }) {
  const [draft, setDraft] = React.useState(value)
  const errors = validateFilterGroup(draft, fields)
  return <SheetContent className="w-full sm:max-w-3xl"><SheetHeader><SheetTitle>Filter contacts</SheetTitle><SheetDescription>Build nested rules across standard and custom fields. Invalid, incomplete, and duplicate conditions are blocked.</SheetDescription></SheetHeader><Separator /><ScrollArea className="min-h-0 flex-1"><div className="flex flex-col gap-3 p-4"><Group group={draft} fields={fields} root={draft} change={setDraft} />{draft.rules.length === 0 && <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No conditions yet. Add a rule to narrow your contacts.</div>}{errors.length > 0 && <div role="alert" className="rounded-lg border border-destructive/30 bg-destructive/10 p-3"><p className="text-sm font-medium text-destructive">Resolve {errors.length} filter {errors.length === 1 ? "issue" : "issues"}</p><ul className="mt-2 list-disc pl-5 text-xs text-destructive">{errors.map((error) => <li key={error}>{error}</li>)}</ul></div>}</div></ScrollArea><Separator /><SheetFooter className="flex-row justify-between"><Button variant="ghost" onClick={() => { onClear(); onOpenChange(false) }}>Clear all</Button><div className="flex gap-2"><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={errors.length > 0} onClick={() => { onApply(draft); onOpenChange(false) }}>Apply filters</Button></div></SheetFooter></SheetContent>
}
