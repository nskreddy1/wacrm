  ...
  return (
    <div className="bg-muted/20 flex flex-col gap-4 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3">
        <div className="bg-card text-primary flex size-10 shrink-0 items-center justify-center rounded-lg border">
          <Database className="size-5" />
        </div>
        <div>
          <p className="text-sm font-semibold">Contact field catalogue</p>
          <p className="text-muted-foreground text-xs">
            Create, edit, and remove account-wide contact fields.
          </p>
        </div>
      </div>
      <Button variant="outline" onClick={() => setOpen(true)}>
        <Sparkles data-icon="inline-start" /> Manage fields
      </Button>
...
export function CustomFieldsManager({
  ...
  const { data, mutate } = useSWR<Store>(
    open ? '/api/v1/workspace/contacts?fields=1' : null
  );
  ...
  async function save() {
    ...
    try {
      field = validateFieldDefinition(
        { label, type, options: parsedOptions },
        data?.data.fields ?? [],
        editing?.id
      );
      setFormError('');
      ...
      setFormError(
        error instanceof Error ? error.message : 'Check the field details'
      );
      return;
    }
    setBusy(true);
    try {
      await request(
        editing ? 'PATCH' : 'POST',
        editing
          ? { kind: 'field', id: editing.id, field }
          : { kind: 'field', field }
      );
  ...
  async function remove(field: ContactField) {
    if (!window.confirm(`Delete “${field.label}” and all saved values?`))
      return;
    try {
      await request('DELETE', { kind: 'field', ids: [field.id] });
      await mutate();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[88vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
        <DialogHeader className="border-b px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                Create typed fields used in contact sheets, tables, filters, and
                imports.
              </DialogDescription>
            </div>
          </div>
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="grid gap-6 p-6 md:grid-cols-[1fr_1.25fr]">
            <section className="bg-muted/20 flex flex-col gap-4 rounded-xl border p-4">
              <div>
                ...
                <p className="text-muted-foreground text-xs">
                  Labels must be unique across standard and custom fields.
                </p>
              </div>
              ...
              {(type === 'single_select' || type === 'multi_select') && (
                <label className="flex flex-col gap-2 text-sm font-medium">
                  ...
                  />
                  <span className="text-muted-foreground text-xs font-normal">
                    Comma-separated, case-insensitively unique, up to 100
                    options. {parsedOptions.length}/100 used.
                  </span>
                </label>
              )}
              {formError && (
            ...
            <section className="flex min-w-0 flex-col gap-3">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">Workspace fields</h3>
                  <p className="text-muted-foreground text-xs">
              ...
              {!data ? (
                ...
                <div className="rounded-xl border border-dashed p-8 text-center">
                  ...
                  <p className="mt-3 text-sm font-medium">No custom fields</p>
                  <p className="text-muted-foreground mt-1 text-xs">
                    Create your first field to extend every contact record.
                  </p>
                </div>
              ) : (
