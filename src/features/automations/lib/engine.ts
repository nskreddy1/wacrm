  ...
  switch (step.step_type) {
    ...
    case 'send_template': {
      ...
      const provider = String(
        (cfg as SendTemplateStepConfig & { template_provider?: string })
          .template_provider ?? 'meta'
      );
      if (provider === 'twilio') {
        const { data: template, error } = await db
          ...
          .maybeSingle();
        if (error || !template)
          throw new Error('Twilio template is missing from this account');
        if (template.status !== 'APPROVED')
          throw new Error('Twilio template is not approved');
        if (!template.twilio_content_sid)
          throw new Error('Twilio template has no Content SID');
