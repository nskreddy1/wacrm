...
import {
  ...
  blankButtonsPayload,
  blankListPayload,
} from '@/features/interactive/components/interactive-builder';
...
function SendTemplateFields({
  ...
  const toValue = (name: string, lang: string, source: string) =>
    `${source}::${name}::${lang}`;
  const current = templateName
    ? toValue(templateName, language, provider || 'meta')
    : '';
  const hasMatch = templates.some(
    (t) =>
      toValue(t.name, t.language ?? 'en_US', t.provider ?? 'meta') === current
  );
      ...
      >
        ...
        {templates.map((tmpl) => {
          ...
          return (
            <option
              key={tmpl.id}
              value={toValue(tmpl.name, lang, tmpl.provider ?? 'meta')}
            >
...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  return (
    <div className="bg-background flex h-full min-h-0 flex-col">
      <header className="border-border bg-card flex shrink-0 flex-wrap items-center gap-2 border-b px-3 py-2 sm:flex-nowrap sm:gap-3 sm:px-4">
        <Button
          type="button"
          variant="ghost"
          size="icon"
        ...
        <div className="min-w-0 flex-1">
          ...
          <input
            ...
            className="text-foreground placeholder:text-muted-foreground focus:bg-muted w-full truncate rounded-md bg-transparent px-2 py-1 text-sm font-semibold focus:outline-none sm:text-base"
          />
          <p className="text-muted-foreground hidden px-2 text-xs sm:block">
            {state.steps.length === 0
              ? 'Trigger only'
              : `${state.steps.length} top-level steps`}
          </p>
        </div>
        <div className="border-border bg-muted flex items-center gap-2 rounded-md border px-2 py-1">
      ...
      <div className="bg-muted/30 relative min-h-0 flex-1 overflow-auto">
