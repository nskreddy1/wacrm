...
import { TEMPLATE_VARIABLES } from "@/lib/templates/studio-types"
...
function categoryFromDb(channel: "whatsapp" | "sms", category: string): TemplateCategory {
  if (channel === "sms") {
...
function buttonsFromDb(row: DbTemplateRow): TemplateButton[] {
  ...
  return row.buttons.flatMap((b, i): TemplateButton[] => {
    const id = `db-${i}`
    if (b.type === "QUICK_REPLY") return [{ id, kind: "quick_reply", label: b.text }]
    if (b.type === "URL") return [{ id, kind: "url", label: b.text, url: b.url ?? "" }]
    if (b.type === "PHONE_NUMBER") return [{ id, kind: "call", label: b.text, phone: b.phone_number ?? "" }]
...
function rowToStudio(row: DbTemplateRow): StudioTemplate {
  return {
    ...
    whatsapp: {
      headerKind: row.channel === "whatsapp" && row.header_type ? (row.header_type as "text" | "image") : "none",
 ...
 */
export function toNumberedVariables(text: string): { text: string; samples: string[] } {
  const seen: string[] = []
  const converted = text.replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}/g, (_, token: string) => {
  ...
  const samples = seen.map(
    (token) => TEMPLATE_VARIABLES.find((v) => v.token === `{{${token}}}`)?.sample ?? token.replace(/_/g, " "),
  )
  ...
  return buttons.map((b) =>
    b.kind === "quick_reply"
      ...
      : b.kind === "url"
        ...
        : { type: "PHONE_NUMBER" as const, text: b.label, phone_number: b.phone },
  )
}

const WHATSAPP_CATEGORY: Record<TemplateCategory, "Marketing" | "Utility" | "Authentication"> = {
  marketing: "Marketing",
  utility: "Utility",
  authentication: "Authentication",
}

const SMS_CATEGORY: Record<TemplateCategory, "marketing" | "transactional" | "otp"> = {
  marketing: "marketing",
...
function saveBody(tpl: StudioTemplate) {
  ...
  return {
    ...
    language: tpl.language,
    header_type: tpl.whatsapp.headerKind === "none" ? null : tpl.whatsapp.headerKind,
    header_content: header ? header.text : tpl.whatsapp.headerKind === "text" ? tpl.whatsapp.headerText : null,
    body_text: body.text,
    footer_text: tpl.whatsapp.footer.trim() || null,
    buttons: tpl.whatsapp.buttons.length ? buttonsToApi(tpl.whatsapp.buttons) : null,
    sample_values:
      body.samples.length || header?.samples.length
        ? { body: body.samples, ...(header?.samples.length ? { header: header.samples } : {}) }
        : null,
    provider: tpl.provider === "twilio" ? ("twilio" as const) : ("meta" as const),
  }
}

/** Payload for the Meta submit + Twilio content routes. */
function submitPayload(tpl: StudioTemplate) {
  const base = saveBody(tpl) as Extract<ReturnType<typeof saveBody>, { channel: "whatsapp" }>
  return {
    name: base.name,
...
export function useStudioTemplates() {
  const { data, error, isLoading, mutate } = useSWR<{ templates: DbTemplateRow[] }>("/api/templates", {
