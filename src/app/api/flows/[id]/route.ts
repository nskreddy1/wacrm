  ...
  if (existingFlow.status === 'active') {
    const effectiveFlow = {
      ...
      trigger_type: (body.trigger_type ??
        existingFlow.trigger_type) as PutBody['trigger_type'] & string,
      trigger_config: (body.trigger_config ??
        existingFlow.trigger_config) as Record<string, unknown>,
      entry_node_id:
        body.entry_node_id !== undefined
          ? body.entry_node_id
          : existingFlow.entry_node_id,
    };
    ...
    if (issues.some((issue) => issue.severity === 'error')) {
      return NextResponse.json(
        {
          error: 'Cannot keep an active flow with invalid configuration',
          issues,
        },
        { status: 422 }
      );
