...
describe('isWithinAutoReplySchedule', () => {
  ...
  it('window boundaries: start inclusive, end exclusive', () => {
    ...
    expect(isWithinAutoReplySchedule(cfg('09:00', '18:00'), atStart)).toBe(true)
  ...
  it('overnight window spans midnight', () => {
    ...
    expect(isWithinAutoReplySchedule(cfg('20:00', '06:00'), midday)).toBe(false)
  })
  ...
  it('evaluates in the configured timezone', () => {
    ...
    expect(
      isWithinAutoReplySchedule(
        cfg('09:00', '18:00', 'Asia/Kolkata'),
        laterUtc,
      ),
    ).toBe(false)
  })
