...
describe('resolveChatModel', () => {
  it('dispatches native providers to their LangChain class', () => {
    expect(resolveChatModel(config({ provider: 'openai' }))).toBeInstanceOf(ChatOpenAI)
    expect(
...
describe('normalizeLcUsage', () => {
  ...
  it('ignores negative / non-finite counts', () => {
    expect(
      normalizeLcUsage({ input_tokens: -5, output_tokens: Number.NaN, total_tokens: 12 }),
    ).toEqual({ promptTokens: 0, completionTokens: 0, totalTokens: 12 })
  })
})

describe('toAiError', () => {
  const status = (n: number) => Object.assign(new Error(`http ${n}`), { status: n })
