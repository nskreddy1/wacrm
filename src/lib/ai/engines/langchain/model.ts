...
// bring-your-own `custom` endpoint all go through `ChatOpenAI` with the
// right base URL. Keys stay per-account (BYO) — passed at call time,
// never from env. `maxRetries: 0` matches the direct adapters'
// single-attempt fetch; the per-call timeout is applied via
// `AbortSignal` at invoke time in `generateLangchain`.
// ============================================================

// Error mapping + labels are engine-agnostic; re-exported for
// convenience so LangChain-engine internals/tests keep one import.
export { providerLabel, toAiError } from '../../errors'
