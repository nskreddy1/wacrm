...
describe("pipeline UUID validation", () => {
  ...
  it("accepts deal relationships backed by Supabase UUID columns", () => {
    ...
    expect(result).toMatchObject({ pipelineId, stageId, contactId, assignedTo: ownerId, currency: "USD" })
  })
  ...
  it("formats validation failures for people instead of serializing Zod issues", () => {
    ...
    if (!result.success) {
      expect(formatPipelineError(result.error)).toBe("stageId: Select a valid record and try again")
    }
  })
})
