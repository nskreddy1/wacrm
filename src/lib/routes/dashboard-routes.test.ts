...
describe('dashboard routes', () => {
  it('builds simple canonical module URLs', () => {
    expect(contactsPath(accountId, { mode: 'sheet', view: viewId })).toBe(
      `/contacts?mode=sheet&view=${viewId}`
    );
    expect(
      pipelinePath(accountId, pipelineId, 'sheet', {
        subPipeline: subPipelineId,
        savedView: viewId,
      })
    ).toBe(
      `/pipelines?pipeline=${pipelineId}&view=sheet&sub_pipeline=${subPipelineId}&saved_view=${viewId}`
    );
  });

  it('omits default state from canonical URLs', () => {
    expect(contactsPath(accountId)).toBe('/contacts');
    expect(contactsPath(accountId, { mode: 'list', view: 'all' })).toBe(
      '/contacts'
    );
    expect(pipelinePath(accountId, pipelineId)).toBe(
