...
export type UnifiedDeliveryStatus = 'sending' | 'sent' | 'delivered' | 'read' | 'failed'
 ...
 */
export async function applyMessageDeliveryStatus(event: DeliveryStatusEvent): Promise<void> {
  ...
  if (recErr) {
  ...
  } else if (recipient && isValidStatusTransition(recipient.status, event.status)) {
    ...
    const { error: updErr } = await db
      ...
      .update(update)
      .eq('id', recipient.id)
    if (updErr) console.error('[status] broadcast recipient update failed:', updErr.message)
  }
