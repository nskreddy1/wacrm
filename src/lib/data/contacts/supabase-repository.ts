...
function fieldOptionsPayload(definition: {
  options: string[];
  required?: boolean;
  unique?: boolean;
}) {
...
function mapContact(
  ...
  customValues: Record<string, ContactValue> = {}
): WorkspaceContact {
  return {
    id: String(row.id),
...
async function resolveExtendedValues(
  ctx: AccountContext,
  values: Partial<Record<string, ContactValue>>
) {
  const entries = Object.entries(EXTENDED_CONTACT_FIELDS).filter(
  ...
  const byLabel = new Map(
    (rows ?? []).map((row) => [
      String(row.field_name).toLocaleLowerCase(),
      String(row.id),
    ])
  );
...
async function saveCustomValues(
  ...
  const { data: fieldRows, error: fieldsError } = await ctx.supabase
    .from('custom_fields')
  ...
  const rows = entries.map(([custom_field_id, value]) => {
    ...
    const stored = Array.isArray(validated)
      ? validated.join(', ')
      : String(validated ?? '');
    if (field.required && !stored.trim())
  ...
  const uniqueRows = rows.filter(
    (row) => fields.get(row.custom_field_id)?.unique && row.value.trim()
  );
  if (uniqueRows.length) {
    const { data: existing, error: uniqueError } = await ctx.supabase
      ...
      .in(
        'custom_field_id',
        uniqueRows.map((row) => row.custom_field_id)
      )
    ...
    for (const row of uniqueRows) {
      const clash = (existing ?? []).find(
        (item) =>
          String(item.custom_field_id) === row.custom_field_id &&
          String(item.value ?? '')
            .trim()
            .toLocaleLowerCase() === row.value.trim().toLocaleLowerCase()
      );
      if (clash)
        throw new Error(
          `Another contact already uses this ${fields.get(row.custom_field_id)!.label} value`
        );
    }
  }
...
export async function getSupabaseContactWorkspace(
  ctx: AccountContext
): Promise<ContactWorkspaceData> {
  const [contactsResult, fieldsResult, valuesResult, profilesResult] =
    await Promise.all([
      ctx.supabase
        .from('contacts')
  ...
  const fields = [
    ...coreContactFields,
    ...(fieldsResult.data ?? []).map((row) => mapField(row)),
  ];
  ...
  for (const field of fields) {
    ...
    const alias = Object.entries(EXTENDED_CONTACT_FIELDS).find(
      ([, label]) =>
        label.toLocaleLowerCase() === field.label.toLocaleLowerCase()
    )?.[0];
  ...
  return {
    contacts: (contactsResult.data ?? []).map((row) => {
      ...
      const aliases = Object.fromEntries(
        [...aliasByFieldId]
          .filter(([fieldId]) => customValues[fieldId] !== undefined)
          .map(([fieldId, alias]) => [alias, customValues[fieldId]])
      );
  ...
  return mapContact(
    ...
    Object.fromEntries(
      Object.entries(resolved).filter(
        (entry): entry is [string, ContactValue] => entry[1] !== undefined
      )
    )
  );
}

export async function updateSupabaseContact(
  ctx: AccountContext,
  id: string,
  values: Partial<Record<string, ContactValue>>
) {
  ...
  const identityTouched = ['name', 'phone', 'email', 'company'].some(
  ...
  const mergedValues = Object.fromEntries(
    Object.entries({ ...currentContact.values, ...resolved }).filter(
      (entry): entry is [string, ContactValue] => entry[1] !== undefined
    )
  );
...
export async function createSupabaseContactField(
  ctx: AccountContext,
  field: {
    label: string;
    type: FieldType;
  ...
  const { data: rows, error: fieldsError } = await ctx.supabase
    .from('custom_fields')
    .select('id, field_name, field_type, field_options')
    .eq('account_id', ctx.accountId);
  if (fieldsError) throw new Error(fieldsError.message);
  const validated = validateFieldDefinition(field, [
    ...coreContactFields,
    ...(rows ?? []).map(mapField),
  ]);
  ...
  if (error)
    throw new Error(
      error.code === '23505'
        ? `A field named “${validated.label}” already exists`
        : error.message
    );
  return mapField(data);
}

export async function updateSupabaseContactField(
  ctx: AccountContext,
  id: string,
  field: {
    label?: string;
  ...
  const validated = validateFieldDefinition(
    {
      label: field.label ?? current.label,
      type: field.type ?? current.type,
      options: field.options ?? current.options,
    },
    [...coreContactFields, ...existing],
    id
  );
  const payload = fieldOptionsPayload({
    ...validated,
    required: field.required ?? current.required === true,
    unique: field.unique ?? current.unique === true,
  });
  const { data, error } = await ctx.supabase
    .from('custom_fields')
    .update({
...
export async function deleteSupabaseContactFields(
  ctx: AccountContext,
  ids: string[]
) {
...
export async function deleteSupabaseContacts(
  ctx: AccountContext,
  ids: string[]
) {
