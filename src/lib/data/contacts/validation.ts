...
export const RESERVED_CONTACT_FIELD_LABELS = new Set([
  'title',
  'description',
  'street',
  'city',
...
export function isReservedContactField(label: string) {
  return RESERVED_CONTACT_FIELD_LABELS.has(
    normalizeFieldLabel(label).toLocaleLowerCase()
  );
}
...
export function validateFieldDefinition(
  ...
  if (CORE_LABELS.has(normalizedLabel))
    throw new Error(`“${label}” is already a standard contact field`);
  if (
    existing.some(
      (field) =>
        field.id !== editingId &&
        normalizeFieldLabel(field.label).toLocaleLowerCase() === normalizedLabel
    )
  ) {
    throw new Error(`A field named “${label}” already exists`);
  }
  if (!FIELD_TYPES.includes(input.type as FieldType))
    throw new Error('Choose a valid field type');
  ...
  if (options.length > 100)
    throw new Error('Select fields support up to 100 options');
...
export function validateContactIdentity(
  ...
  if (name.length > 120)
    throw new Error('Contact name must be 120 characters or fewer');
  ...
  if (company.length > 160)
    throw new Error('Company must be 160 characters or fewer');
  return { name, phone, email, company };
}

export function validateContactValue(
  field: ContactField,
  value: ContactValue | null | undefined
): ContactValue {
