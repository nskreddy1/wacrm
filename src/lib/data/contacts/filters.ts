...
export const isGroup = (item: FilterRule | FilterGroup): item is FilterGroup =>
  'rules' in item;
...
export function operatorsFor(
  ...
  if (
    ...
    return [
      ...
      {
        value: 'gt',
        label: field.type === 'date' ? 'is after' : 'is greater than',
      },
      { value: 'gte', label: 'is greater than or equal' },
      {
        value: 'lt',
        label: field.type === 'date' ? 'is before' : 'is less than',
      },
...
export function validateFilterGroup(
  ...
  for (const rule of flattenRules(group)) {
    ...
    if (!field) {
      errors.push('A filter references a field that no longer exists');
      continue;
    }
    if (!operatorsFor(field).some((item) => item.value === rule.operator))
      errors.push(`${field.label} uses an incompatible operator`);
    const needsValue = !rule.operator.includes('empty');
    if (needsValue && !rule.value.trim())
      errors.push(`${field.label} needs a value`);
    if (
      needsValue &&
      (field.type === 'number' || field.type === 'currency') &&
      !Number.isFinite(Number(rule.value))
    )
      errors.push(`${field.label} needs a valid number`);
    if (
      needsValue &&
      field.type === 'date' &&
      Number.isNaN(Date.parse(rule.value))
    )
      errors.push(`${field.label} needs a valid date`);
    if (
      needsValue &&
      field.type === 'single_select' &&
      !field.options?.includes(rule.value)
    )
      errors.push(`${field.label} needs one of its configured options`);
    const signature = `${rule.field}|${rule.operator}|${rule.value.trim().toLocaleLowerCase()}`;
    if (signatures.has(signature))
      errors.push(`Duplicate condition for ${field.label}`);
...
export function matchesFilter(
  ...
  const results = group.rules.map((item) =>
    isGroup(item)
      ? matchesFilter(contact, fields, item)
      : evaluateRule(
          contact,
          fields.find((field) => field.id === item.field),
          item
        )
  );
...
export function countRules(group: FilterGroup): number {
  return group.rules.reduce(
    (total, item) => total + (isGroup(item) ? countRules(item) : 1),
    0
  );
}
export function summarizeRule(rule: FilterRule, fields: ContactField[]) {
  const field = fields.find((item) => item.id === rule.field);
  const operator = operatorsFor(field).find(
    (item) => item.value === rule.operator
  )?.label;
  return `${field?.label ?? rule.field} ${operator ?? rule.operator}${rule.operator.includes('empty') ? '' : ` ${rule.value}`}`;
}
export function flattenRules(group: FilterGroup): FilterRule[] {
  return group.rules.flatMap((item) =>
    isGroup(item) ? flattenRules(item) : item
  );
}
