  ...
  return (
    <div className="mt-px flex flex-col gap-px overflow-hidden rounded-b-lg">
      {draft.buttons.map((btn) => (
        ...
        >
          {btn.kind === "url" && <ExternalLink className="size-3.5" aria-hidden="true" />}
          {btn.kind === "call" && <Phone className="size-3.5" aria-hidden="true" />}
          {btn.kind === "quick_reply" && <Reply className="size-3.5" aria-hidden="true" />}
          {btn.label || "Button"}
        </div>
      ))}
    </div>
          ...
          <div className="overflow-hidden rounded-lg rounded-tl-none bg-[#202c33] shadow-sm">
            ...
            <div className="px-2.5 pb-1.5 pt-2">
              {draft.headerKind === "text" && header && (
                <p className="mb-1 text-[13.5px] font-bold leading-snug text-[#e9edef]">{header}</p>
              )}
              <p className="whitespace-pre-wrap text-[13.5px] leading-snug text-[#e9edef]">
                {body || "Start typing to see your message here…"}
              </p>
              {draft.footer && (
                <p className="mt-1 text-[11px] leading-snug text-[#8696a0]">{draft.footer}</p>
              )}
              <p className="mt-0.5 text-right text-[10px] text-[#8696a0]">10:42 AM</p>
            </div>
          </div>
...
function SmsThread({
  ...
  return (
    <div className={cn("flex h-full flex-col", isIphone ? "bg-black" : "bg-[#121212]")}>
      ...
      <div
        className={cn(
          "flex flex-col items-center gap-1 px-3 pb-2 pt-1.5",
      ...
      >
        <div
          className={cn(
            ...
            isIphone ? "size-9 bg-[#8e8e93] text-[13px]" : "size-8 bg-[#7c4dff] text-[12px]",
          )}
        >
          A
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4">
        ...
        <div className="max-w-[85%]">
          <div
            className={cn(
              "w-fit whitespace-pre-wrap px-3 py-2 text-[13.5px] leading-snug text-white",
              isIphone ? "rounded-2xl rounded-bl-md bg-[#3a3a3c]" : "rounded-2xl rounded-bl-sm bg-[#2d2d2d]",
            )}
          >
            {body || "Start typing to see your message here…"}
          </div>
  ...
  return (
    ...
    >
      ...
      {isIphone ? (
        ...
        <div className="absolute left-1/2 top-2 z-10 size-2.5 -translate-x-1/2 rounded-full bg-[#2c2c2e]" />
      )}
