...
export function CatalogWorkspace() {
  ...
  const stats = useMemo(() => {
    ...
    const averagePrice = active.length > 0 ? active.reduce((sum, item) => sum + item.price, 0) / active.length : 0
    return {
      total: items.length,
  ...
  const kpis = [
    { label: "Total items", value: String(stats.total), Icon: Package, note: "Everything in your catalog" },
    { label: "Active", value: String(stats.active), Icon: CircleDollarSign, note: "Bookable right now" },
    ...
    <main className="flex min-h-full flex-col gap-6 p-4 md:p-6">
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-balance text-2xl font-semibold tracking-tight text-foreground md:text-3xl">Catalog</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Catalog overview">
        ...
        <CardHeader className="border-b border-border">
          <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Services and products available for scheduling</p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <Select
                items={{ all: "All statuses", active: "Active", archived: "Archived" }}
                value={statusFilter}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="flex items-center justify-center gap-2 p-16 text-sm text-muted-foreground">
              <Loader2 className="size-4 animate-spin" aria-hidden="true" /> Loading catalog
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-3 p-16 text-center">
              ...
              <div>
                <p className="font-medium text-foreground">No catalog items found</p>
                <p className="text-sm text-muted-foreground">Create your first item or adjust the filters.</p>
              </div>
