...
const dueFormatter = new Intl.DateTimeFormat("en", { month: "short", day: "numeric" })
...
export function TasksPanel({ tasks, onChanged }: { tasks: OpenTask[]; onChanged: () => void }) {
  ...
  return (
    <ChartCard title="Tasks" caption="Open follow-ups, most urgent first" contentClassName="flex flex-col gap-1 p-0">
      <form
      ...
      >
        ...
        />
        <Button type="submit" variant="ghost" size="sm" disabled={!title.trim() || busy} className="h-7 gap-1 px-2 text-xs font-medium text-primary hover:text-primary">
      ...
      {tasks.length === 0 ? (
        <p className="px-4 py-5 text-center text-xs text-muted-foreground">No open tasks. Nice work.</p>
      ) : (
            ...
            <li key={task.id} className="flex items-center gap-3 px-4 py-2">
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-0.5 flex items-center gap-2 text-[11px] text-muted-foreground">
                  {task.contact && <span className="truncate">{task.contact}</span>}
                  {task.dueAt && (
                    <span className={cn("shrink-0 tabular-nums", task.overdue && "font-semibold text-destructive")}>
              ...
              <span className={cn("shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize", PRIORITY_BADGE[task.priority])}>
                {task.priority}
              </span>
            </li>
          ))}
