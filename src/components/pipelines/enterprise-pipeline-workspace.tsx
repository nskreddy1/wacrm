...
export function EnterprisePipelineWorkspace({ initialDeals = demoDeals }: { initialDeals?: DemoDeal[] }) {
  ...
  return <div className="flex h-[calc(100vh-3.5rem)] min-h-0 flex-col overflow-hidden bg-background">
    <header className="flex min-h-14 flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
      ...
      <div className="flex items-center rounded-full bg-muted p-1"><FieldPicker label={view === "board" ? "Card fields" : "List columns"} selected={view === "board" ? cardFields : visibleFields} onToggle={(field) => toggleField(field, view === "board" ? "card" : "list")} icon={view === "board" ? "sliders" : "columns"} /><Tabs value={view} onValueChange={(value) => setView(value as ViewMode)}><TabsList className="bg-transparent"><TabsTrigger value="board" aria-label="Board view"><LayoutGrid /></TabsTrigger><TabsTrigger value="list" aria-label="List view"><List /></TabsTrigger></TabsList></Tabs></div>
