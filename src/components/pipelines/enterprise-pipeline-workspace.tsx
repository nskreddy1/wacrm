...
type DealField = keyof Pick<DemoDeal, "title" | "value" | "stageId" | "due" | "company" | "contact" | "owner" | "priority" | "probability" | "createdAt" | "source" | "activity">
...
export function EnterprisePipelineWorkspace() {
  ...
  function exportCsv() { const rows = [visibleFields.map((field) => fields.find((item) => item.id === field)?.label), ...filtered.map((deal) => visibleFields.map((field) => displayValue(deal, field)))]; const csv = rows.map((row) => row.map((value) => `"${String(value).replaceAll('"', '""')}"`).join(",")).join("\n"); const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" })); link.download = "pipeline-deals.csv"; link.click() }
  ...
  return <div className="flex min-h-[calc(100vh-3.5rem)] flex-col overflow-hidden bg-background">
    <header className="flex flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
      <Button variant={(ownerFilter !== "all" || stageFilter !== "all") ? "secondary" : "outline"} size="icon-sm" aria-label="Filter deals" onClick={() => setOwnerFilter((value) => value === "all" ? "Sam Silva" : "all")}><Filter /></Button>
      ...
      <div className="relative min-w-44 flex-1 md:max-w-sm"><Search className="absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0) }} placeholder="Search deals, contacts, companies" className="pl-8" /></div>
      <Select value={ownerFilter} onValueChange={(value) => value && setOwnerFilter(value)}><SelectTrigger size="sm" className="hidden md:flex"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">All owners</SelectItem>{["Sam Silva", "Nora James", "Ravi Patel"].map((owner) => <SelectItem key={owner} value={owner}>{owner}</SelectItem>)}</SelectGroup></SelectContent></Select>
      <Select value={stageFilter} onValueChange={(value) => value && setStageFilter(value)}><SelectTrigger size="sm" className="hidden lg:flex"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">All stages</SelectItem>{demoStages.map((stage) => <SelectItem key={stage.id} value={stage.id}>{stage.name}</SelectItem>)}</SelectGroup></SelectContent></Select>
      <FieldPicker label="Card fields" selected={cardFields} onToggle={(field) => toggleField(field, "card")} icon="sliders" />
      <FieldPicker label="List columns" selected={visibleFields} onToggle={(field) => toggleField(field, "list")} icon="columns" />
      <Tabs value={view} onValueChange={(value) => setView(value as ViewMode)}><TabsList><TabsTrigger value="board" aria-label="Board view"><LayoutGrid /></TabsTrigger><TabsTrigger value="list" aria-label="List view"><List /></TabsTrigger></TabsList></Tabs>
      <Button size="sm" onClick={() => openDeal()}><Plus data-icon="inline-start" /> Deal</Button>
      <DropdownMenu><DropdownMenuTrigger render={<Button variant="outline" size="icon-sm" aria-label="More pipeline actions" />}><Ellipsis /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuGroup><DropdownMenuItem onClick={exportCsv}><Download /> Export CSV</DropdownMenuItem><DropdownMenuItem onClick={() => setCreateViewOpen(true)}><Star /> Save current view</DropdownMenuItem></DropdownMenuGroup></DropdownMenuContent></DropdownMenu>
    </header>
    ...
    <Dialog open={createViewOpen} onOpenChange={setCreateViewOpen}><DialogContent><DialogHeader><DialogTitle>Create saved view</DialogTitle><DialogDescription>Save the current owner, stage, fields, sorting and workspace mode for your sales team.</DialogDescription></DialogHeader><div className="flex flex-col gap-2"><Label htmlFor="view-name">View name</Label><Input id="view-name" value={newViewName} onChange={(event) => setNewViewName(event.target.value)} placeholder="Enterprise renewals" /></div><DialogFooter><Button variant="outline" onClick={() => setCreateViewOpen(false)}>Cancel</Button><Button onClick={() => { if (!newViewName.trim()) return; const next = { id: `view-${Date.now()}`, name: newViewName, filter: "all" as const }; setViews((current) => [...current, next]); setActiveViewId(next.id); setNewViewName(""); setCreateViewOpen(false); toast.success("View created") }}>Create view</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
...
function StageColumn({ stage, deals, cardFields, onOpen }: { stage: DemoStage; deals: DemoDeal[]; cardFields: DealField[]; onOpen: (deal?: DemoDeal, stageId?: string) => void }) {
  ...
  return <section ref={setNodeRef} className={cn("flex h-full w-72 flex-col overflow-hidden rounded-lg border bg-card/70 transition-colors", isOver && "border-primary bg-primary/5")}><div className={cn("h-1 shrink-0", stageTone[stage.color])} /><header className="border-b bg-card px-3 py-2.5"><div className="flex items-start justify-between gap-2"><div><h2 className="text-sm font-semibold">{stage.name}</h2><p className="text-xs text-muted-foreground">${deals.reduce((sum, deal) => sum + deal.value, 0).toLocaleString()} · {deals.length} {deals.length === 1 ? "deal" : "deals"}</p></div><Button variant="ghost" size="icon-sm" aria-label={`${stage.name} options`}><Ellipsis /></Button></div><p className="mt-2 truncate text-xs text-muted-foreground">{stage.description}</p></header><ScrollArea className="min-h-0 flex-1"><div className="flex flex-col gap-2 p-2">{deals.map((deal) => <DraggableDeal key={deal.id} deal={deal} fields={cardFields} onOpen={onOpen} />)}{deals.length === 0 && <div className="flex min-h-36 items-center justify-center rounded-md border border-dashed text-xs text-muted-foreground">Drop deals here</div>}<Button variant="ghost" className="justify-start border border-dashed" onClick={() => onOpen(undefined, stage.id)}><Plus data-icon="inline-start" /> Add deal</Button></div></ScrollArea></section>
}
...
function DealCard({ deal, fields: shownFields, onOpen, dragProps, overlay }: { deal: DemoDeal; fields: DealField[]; onOpen: (deal: DemoDeal) => void; dragProps?: Record<string, unknown>; overlay?: boolean }) { return <article className={cn("rounded-lg border bg-card p-3 shadow-xs hover:shadow-sm", overlay && "w-72 shadow-lg")}><div className="flex items-start gap-2"><button aria-label={`Drag ${deal.title}`} className="mt-0.5 cursor-grab text-muted-foreground" {...dragProps}><GripVertical className="size-4" /></button><button className="min-w-0 flex-1 truncate text-left text-sm font-semibold hover:text-primary" onClick={() => onOpen(deal)}>{deal.title}</button><Badge variant={deal.priority === "Hot" ? "destructive" : "secondary"}>{deal.priority}</Badge></div><div className="mt-2 flex flex-col gap-1.5">{shownFields.filter((field) => !["title", "priority", "stageId"].includes(field)).slice(0, 6).map((field) => <div key={field} className="flex items-center gap-2 text-xs"><span className="w-20 shrink-0 text-muted-foreground">{fields.find((item) => item.id === field)?.label}</span><span className="truncate font-medium">{displayValue(deal, field)}</span></div>)}</div><div className="mt-3 flex items-center justify-between border-t pt-2 text-xs text-muted-foreground"><span className="flex items-center gap-1"><CalendarClock className="size-3.5" /> {deal.activity}</span><Avatar className="size-6"><AvatarFallback>{deal.owner.split(" ").map((part) => part[0]).join("")}</AvatarFallback></Avatar></div></article> }
