...
export function PipelineWorkspace({ initialSnapshot, initialMode, initialSubPipelineId, initialSavedViewId }: { initialSnapshot: PipelineSnapshot; initialMode: PipelineMode; initialSubPipelineId?: string; initialSavedViewId?: string }) {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
    ...
    <footer className="flex min-h-12 shrink-0 flex-wrap items-center gap-x-4 gap-y-2 border-t bg-card px-4 py-2 text-xs shadow-xs" aria-label="Pipeline summary">
      <span><strong>{deals.length}</strong> deals</span>
      <span><strong>{deals.filter((deal) => deal.status === "open").length}</strong> open</span>
      <span><strong>{new Intl.NumberFormat(undefined, { style: "currency", currency: deals[0]?.currency ?? "USD", maximumFractionDigits: 0 }).format(deals.reduce((sum, deal) => sum + deal.value, 0))}</strong> value</span>
      {(owner !== "all" || stage !== "all") && <Button variant="ghost" size="sm" onClick={() => { setOwner("all"); setStage("all") }}><X data-icon="inline-start" />Clear filters</Button>}
    </footer>
