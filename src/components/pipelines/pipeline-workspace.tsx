  ...
  return (
    <div className="bg-background flex min-h-0 flex-1 flex-col overflow-hidden">
      <header className="bg-card flex shrink-0 flex-wrap items-center gap-2 border-b px-3 py-2 lg:px-4">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Narrow this board by owner or stage.
                </p>
              </div>
              <Select
                items={{
        ...
        <Button onClick={() => createInStage(snapshot.stages[0]?.id ?? '')}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuGroup>
              <DropdownMenuItem onClick={() => setAscending((value) => !value)}>
                {ascending ? <ArrowUp /> : <ArrowDown />}Sort{' '}
                {ascending ? 'ascending' : 'descending'}
              </DropdownMenuItem>
              {(['createdAt', 'value', 'due'] as SortKey[]).map((key) => (
      ...
      {deals.length === 0 && (query || activeFilterCount > 0) ? (
        ...
        >
          <div className="bg-muted/20 grid min-h-0 flex-1 grid-flow-row grid-cols-1 gap-3 overflow-y-auto p-3 sm:auto-cols-[minmax(17rem,1fr)] sm:grid-flow-col sm:grid-cols-none sm:overflow-x-auto sm:overflow-y-hidden">
            {snapshot.stages.map((item) => (
              <StageColumn
                key={item.id}
                stage={item}
...
function StageColumn({
  ...
  return (
    <section
      ref={setNodeRef}
      className={cn(
        'bg-card flex min-h-0 flex-col overflow-hidden rounded-xl border shadow-xs transition-[box-shadow,border-color] duration-200',
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-2">
        ...
        {deals.length === 0 && (
          <div
            className={cn(
              'm-auto flex w-full flex-col items-center gap-3 rounded-lg border border-dashed p-6 text-center',
...
function DealCard({
  ...
  return (
    <article
      ...
      className={cn(
        'pipeline-deal-card group bg-background rounded-lg border p-3 shadow-xs',
    ...
    >
      <div className="flex items-start gap-2">
        <button
          ...
          className="text-muted-foreground mt-0.5 cursor-grab opacity-60 transition-opacity hover:opacity-100 active:cursor-grabbing"
        >
          <GripVertical className="size-4" />
        </button>
        <button
          className="hover:text-primary min-w-0 flex-1 text-left text-sm leading-5 font-semibold"
          onClick={() => onOpen(deal)}
        >
          {deal.title}
        </button>
      ...
      {(deal.contact?.name || deal.company) && (
        <p className="text-muted-foreground mt-1 truncate text-xs">
          {deal.contact?.name ?? deal.company}
          {deal.contact?.name && deal.company ? ` · ${deal.company}` : ''}
        </p>
      )}
        ...
        {timing && (
          <span
            className={cn(
              ...
              timing.urgent && 'text-destructive font-medium'
            )}
          >
