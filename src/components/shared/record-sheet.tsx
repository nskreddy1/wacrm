...
export function RecordSheet({
  ...
  readonly = false,
  isCreate = false,
  customizeLabel = 'Customize Fields',
  children,
  onOpenChange,
  onSubmit,
  onCancel,
  ...
  return (
    <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-[9rem_minmax(0,1fr)] sm:gap-4">
      {labelSlot ?? (
        <label
          htmlFor={htmlFor}
          className="text-sm font-medium sm:flex sm:w-36 sm:justify-end sm:text-right"
        >
          {label}
        </label>
      )}
...
// ---------------------------------------------------------- RecordCollapsible
export function RecordCollapsible({
  title,
  open,
  onOpenChange,
  children,
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          render={
            <button
              type="button"
              aria-labelledby="record-owner-label"
              disabled={disabled}
      ...
      <PopoverTrigger
        render={
          <button
            ...
            disabled={disabled}
            className={cn(
              'hover:bg-muted/40 focus-visible:ring-ring flex h-11 w-full items-center justify-between gap-2 rounded-md border bg-transparent px-3 text-sm shadow-xs transition-colors focus-visible:ring-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-60',
      ...
      <PopoverContent align="start" className="w-80 gap-0 p-0">
        ...
        >
          {value ? (
            <button
              type="button"
              className="text-muted-foreground hover:bg-muted flex w-full items-center rounded-sm px-3 py-2 text-left text-sm transition-colors"
              onClick={() => {
                onSelect(null);
                setOpen(false);
              }}
          ...
          {allowCustom && trimmedQuery && !exactMatch ? (
            <button
              type="button"
              className="text-primary hover:bg-muted flex w-full items-center gap-2 rounded-sm px-3 py-2 text-left text-sm font-medium transition-colors"
              onClick={() => {
          ...
          {filtered.length === 0 && !(allowCustom && trimmedQuery) ? (
            ...
            filtered.map((option) => (
              <button
                ...
                className={cn(
                  'hover:bg-muted flex w-full items-center justify-between gap-2 rounded-sm px-3 py-2 text-left text-sm transition-colors',
                  option.id === value && 'bg-muted/60 font-medium'
                )}
                onClick={() => {
                  onSelect(option.id);
                  setOpen(false);
        ...
        >
          <DialogHeader className="border-b px-6 py-4 text-left">
            ...
            <DialogDescription className="sr-only">
              Quickly create a {entity.toLowerCase()} without leaving this form
            </DialogDescription>
          </DialogHeader>
