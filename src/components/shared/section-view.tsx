...
export function SectionTabs({ tabs, active, onSelect, className }: {
  ...
  return (
    <div role="tablist" aria-label="Section tabs" className={cn("flex items-center gap-6 border-b", className)}>
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={cn(
              "-mb-px inline-flex items-center gap-1.5 border-b-2 px-0.5 pb-2.5 pt-1 text-sm font-medium transition-colors",
              isActive
          ...
          >
            {tab.label}
            {tab.badge != null ? <span className="text-xs text-muted-foreground">{tab.badge}</span> : null}
          </button>
        )
      })}
    </div>
...
export function FilterChips({ chips, active, onSelect, className }: {
  chips: FilterChip[]
  active: string
  onSelect: (id: string) => void
  className?: string
  ...
  return (
    <div role="group" aria-label="Filters" className={cn("inline-flex items-center gap-1 rounded-full border p-1", className)}>
      {chips.map((chip) => {
 ...
 * ---------------------------------------------------------------- */
export function SectionToolbar({ left, search, onSearchChange, searchPlaceholder = "Search", action, className }: {
  left?: ReactNode
  search?: string
...
export function DataTable<Row>({ columns, rows, rowKey, empty, onRowClick, className }: {
