              ...
              <CardContent className="flex flex-col gap-5">
                ...
                {!providers.find((item) => item.provider === provider)?.available ? <Alert><AlertTitle>Not available in this slice</AlertTitle><AlertDescription>This provider remains selectable in the architecture, but setup is disabled until its real authentication flow is implemented and tested.</AlertDescription></Alert> : (
                  <>
                    ...
                    {provider === 'twilio' ? <div className="grid gap-4 md:grid-cols-2"><div className="flex flex-col gap-2"><Label htmlFor="twilio-sid">Account SID</Label><Input id="twilio-sid" value={form.accountSid} onChange={(event) => update('accountSid', event.target.value)} /></div><div className="flex flex-col gap-2"><Label htmlFor="twilio-token">Auth token</Label><Input id="twilio-token" type="password" value={form.authToken} onChange={(event) => update('authToken', event.target.value)} /></div>{tab === 'sms' ? <div className="flex flex-col gap-2 md:col-span-2"><Label htmlFor="twilio-messaging-service">Messaging Service SID <span className="font-normal text-muted-foreground">(optional, recommended)</span></Label><Input id="twilio-messaging-service" value={form.messagingServiceSid} onChange={(event) => update('messagingServiceSid', event.target.value)} placeholder="MGxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" /><p className="text-xs text-muted-foreground">When set, sends use Twilio&apos;s Messaging Service for sender pooling, Sticky Sender, and carrier-managed STOP handling (Advanced Opt-Out). Leave blank to send from the number above.</p></div> : null}</div> : null}
                  </>
                )}
              </CardContent>
