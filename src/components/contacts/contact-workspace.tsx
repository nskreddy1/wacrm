  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        {view === "cards" && <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">{rows.map((contact) => <article key={contact.id} className="flex flex-col gap-4 rounded-xl border bg-card p-4 shadow-xs"><div className="flex items-start gap-3"><Checkbox checked={selected.has(contact.id)} onCheckedChange={() => toggleSelected(contact.id)} aria-label={`Select ${valueText(contact.values.name)}`} /><Avatar><AvatarFallback>{valueText(contact.values.name).split(" ").map((part) => part[0]).slice(0, 2).join("")}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><button className="truncate font-semibold hover:text-primary" onClick={() => router.push(contactsPath(undefined, { contact: contact.id, view: savedViewId, mode: view }))}>{valueText(contact.values.name)}</button><p className="truncate text-sm text-muted-foreground">{valueText(contact.values.company)}</p></div><Badge variant="secondary">{valueText(contact.values.lifecycle)}</Badge></div><div className="grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted-foreground">Email</p><p className="truncate">{valueText(contact.values.email)}</p></div><div><p className="text-xs text-muted-foreground">Phone</p><p>{valueText(contact.values.phone)}</p></div><div><p className="text-xs text-muted-foreground">Location</p><p>{valueText(contact.values.city)}</p></div><div><p className="text-xs text-muted-foreground">Value</p><p>${Number(contact.values.value ?? 0).toLocaleString()}</p></div></div></article>)}</div>}
      </div>
      ...
      <Sheet open={Boolean(detail)} onOpenChange={(open) => { if (!open) router.push(contactsPath(undefined, { mode: view, view: savedViewId })) }}><SheetContent className="sm:max-w-lg"><SheetHeader><SheetTitle>{detail ? valueText(detail.values.name) : "Contact details"}</SheetTitle><SheetDescription>Enterprise contact record</SheetDescription></SheetHeader>{detail && <ScrollArea className="h-[calc(100vh-8rem)]"><div className="flex flex-col gap-4 p-4">{orderedFields.map((field) => <div key={field.id} className="flex items-start justify-between gap-4 border-b pb-3"><span className="text-sm text-muted-foreground">{field.label}</span><span className="max-w-64 text-right text-sm font-medium">{valueText(detail.values[field.id]) || "—"}</span></div>)}</div></ScrollArea>}</SheetContent></Sheet>
    </div>
  )
}
