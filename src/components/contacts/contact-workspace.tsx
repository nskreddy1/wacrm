...
export function ContactWorkspace({ initialView = "list", savedViewId = "all", initialContactId }: { initialView?: View; savedViewId?: string; initialContactId?: string }) {
  ...
  const detail = store?.contacts.find((contact) => contact.id === initialContactId) ?? null
  ...
  return (
    <div className="flex min-h-full flex-col bg-background">
      <div className="flex flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
        ...
        <Button size="sm" onClick={() => router.push("/contacts/new")}><Plus data-icon="inline-start" /> Contact</Button>
      ...
      <Sheet open={Boolean(detail)} onOpenChange={(open) => { if (!open) router.push(contactsPath(undefined, { mode: view, view: savedViewId })) }}><SheetContent className="sm:max-w-lg"><SheetHeader><SheetTitle>{detail ? valueText(detail.values.name) : "Contact details"}</SheetTitle><SheetDescription>Enterprise contact record</SheetDescription>{detail && <Button size="sm" onClick={() => router.push(`/contacts/${detail.id}/edit`)}>Edit contact</Button>}</SheetHeader>{detail && <ScrollArea className="h-[calc(100vh-8rem)]"><div className="flex flex-col gap-4 p-4">{orderedFields.map((field) => <div key={field.id} className="flex items-start justify-between gap-4 border-b pb-3"><span className="text-sm text-muted-foreground">{field.label}</span><span className="max-w-64 text-right text-sm font-medium">{valueText(detail.values[field.id]) || "—"}</span></div>)}</div></ScrollArea>}</SheetContent></Sheet>
    </div>
  )
}
