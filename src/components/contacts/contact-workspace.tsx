...
export function ContactWorkspace() {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = term ? store.contacts.filter((contact) => Object.values(contact.values).some((value) => valueText(value).toLowerCase().includes(term))) : [...store.contacts]
  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] flex-col bg-background">
      <div className="flex flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
        <Button variant="outline" size="sm"><Filter data-icon="inline-start" /> Filter</Button>
