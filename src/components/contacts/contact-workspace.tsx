...
import { ContactRecordSheet, type ContactSheetState } from "@/components/contacts/contact-record-sheet"
...
export function ContactWorkspace({ initialView = "list", savedViewId = "all", initialContactId }: { initialView?: View; savedViewId?: string; initialContactId?: string }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-background">
      ...
      <ContactRecordSheet state={contactSheet ?? (detail ? { mode: "view", contact: detail } : null)} fields={orderedFields} onOpenChange={(open) => { if (!open) { setContactSheet(null); if (detail) router.push(contactsPath(undefined, { mode: view, view: savedViewId })) } }} onSaved={() => mutate()} />
