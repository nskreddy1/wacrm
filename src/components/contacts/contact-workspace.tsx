  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] flex-col bg-background">
      <div className="flex flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
        ...
        <Tabs value={view} onValueChange={(value) => router.replace(contactsPath(accountId, { mode: value as View, view: savedViewId }))}><TabsList><TabsTrigger value="list" aria-label="List view"><List /></TabsTrigger><TabsTrigger value="sheet" aria-label="Sheet view"><SheetIcon /></TabsTrigger><TabsTrigger value="cards" aria-label="Cards view"><Grid2X2 /></TabsTrigger></TabsList></Tabs>
      ...
      <Sheet open={Boolean(detail)} onOpenChange={(open) => { if (!open) router.push(contactsPath(accountId, { mode: view, view: savedViewId })) }}><SheetContent className="sm:max-w-lg"><SheetHeader><SheetTitle>{detail ? valueText(detail.values.name) : "Contact details"}</SheetTitle><SheetDescription>Enterprise contact record</SheetDescription></SheetHeader>{detail && <ScrollArea className="h-[calc(100vh-8rem)]"><div className="flex flex-col gap-4 p-4">{orderedFields.map((field) => <div key={field.id} className="flex items-start justify-between gap-4 border-b pb-3"><span className="text-sm text-muted-foreground">{field.label}</span><span className="max-w-64 text-right text-sm font-medium">{valueText(detail.values[field.id]) || "—"}</span></div>)}</div></ScrollArea>}</SheetContent></Sheet>
    </div>
  )
}
